...
|-----------|------|----------|-----|---------|
| Compiler correctness | `automatic/Compile/CompileProof.v` | 19 | 125 | Closures, heap allocation, function application, remaining step lemmas needing stronger preconditions; bounded CONSTINT/POP/GETFIELD helpers are proved |
...
**Current state**: 19 Admitted, 125 Qed. Recent progress proved `compiler_correct_empty`, `compiler_correct_type_decl`, `compiler_correct_expr_unit`, `compiler_correct_expr_bool`, `compiler_correct_not_bool`, plus bounded CONSTINT, POP, and GETFIELD helper lemmas. Core blockers are extending `val_corresponds` for closures and strengthening the remaining single-step lemmas with operand bounds/preconditions where handlers reject malformed operands.
