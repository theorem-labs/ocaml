...
Definition pushoffsetclosure2_pre
...
Theorem verify_PUSHOFFSETCLOSURE2_correct :
    handler_correct (handle_PUSHOFFSETCLOSURE 2) f_instr_PUSHOFFSETCLOSURE2
      (fun e m s ard => pushoffsetclosure2_pre e m s ard)
      (fun _ _ => True)
      (fun _ => False)
      (fun _ _ _ => False).
Proof.
  ...
  - simpl.
    ...
    unfold pushoffsetclosure2_pre in Hstep_pre.
    ...
    (* ============================================================== *)
    {
      ...
      (* Statements 6-7: eval_cbn entered the env+accu block.
         After resolving _s for Sset _t'2 (s->env), the composite lookup
         for _env field was already resolved (Hco rewrite from stmt 1).
         We just need to supply the field_offset and continue. *)

      (* Statement 6: resolve _s, field_offset for env, load env *)
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      rewrite Hle_s; eval_cbn.
      ...
      (* Statement 7: Sassign (s->accu) (_t'2 + 3*sizeof(long))
         The rvalue: PTree.gss to resolve _t'2, sem_mul, sem_add, sem_cast.
         The lvalue: PTree.gso for _s, deref struct, field offset at +8.
         The store: comp_assign_loc unfolds to Mem.store. *)

      (* Rvalue part: _t'2 + 3*sizeof(tlong) *)
      rewrite PTree.gss.
      rewrite sem_mul_3_sizeof.
      eval_cbn.
      rewrite sem_add_long_24; eval_cbn.
      rewrite sem_cast_tulong_tlong; eval_cbn.

      (* Lvalue part: s->accu *)
      (* Resolve _s from the temp environment *)
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      ...
      (* Store: comp_assign_loc resolves to Mem.store *)
      rewrite (ptrofs_add_unsigned so 8 ltac:(lia) ltac:(lia)).
      unfold result_v in Hstore_result.
      unfold comp_assign_loc, Mem.storev.
