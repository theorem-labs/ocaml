...
Definition pushenvacc_step_pre_2 (e : Clight.env) (m : mem) (s : Machine.state)
...
Proof.
  ...
  2: { reflexivity. }
  {
    ...
    unfold pushenvacc_step_pre_2 in Hstep_pre. fold sb so hm in Hstep_pre.
    ...
    assert (Hfield_load_m1 : Mem.load Mint64 m1 env_b (Ptrofs.unsigned (Ptrofs.add env_ofs (Ptrofs.repr 16))) = Some cv).
    { erewrite Mem.load_store_other. exact Hfield_load. exact Hstore_sp.
      left. exact Henv_ne_sb. }
    assert (Hfield_load_m2 : Mem.load Mint64 m2 env_b (Ptrofs.unsigned (Ptrofs.add env_ofs (Ptrofs.repr 16))) = Some cv).
...

    {
      ...
      rewrite PTree.gss; eval_cbn.
      rewrite sem_cast_long_to_ptr_vptr_PEA2; eval_cbn.
      rewrite (sem_add_ptr_long_PEA2 env_b env_ofs m2); eval_cbn.
