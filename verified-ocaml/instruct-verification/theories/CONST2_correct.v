...
Proof.
  ...
  destruct (store_succeeds_from_load m sb (Ptrofs.unsigned so + 8) accu_v (Vlong (Int64.repr 5)) Haccu_load) as [m' Hstore].
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    { exists (Vptr sp_b sp_ofs), sp_b, sp_ofs. split; [| split; [| split; [| split; [| split; [| split; [| split; [| split]]]]]]]. exact Hsp_load'. reflexivity. simpl. apply (stack_repr_store_other_block hm m m' _ sp_b sp_ofs sb (uso + 8) (Vlong (Int64.repr 5)) Hstack_repr Hstore). intro Heq; exact (Hsp_ne_sb (eq_sym Heq)).
