...
Proof.
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    { exists (Vptr sp_b sp_ofs), sp_b, sp_ofs. split; [| split; [| split; [| split; [| split]]]]. exact Hsp_load'. reflexivity. simpl. apply (stack_repr_store_other_block hm m m' _ sp_b sp_ofs sb (uso + 8) (Vlong (Int64.repr 5)) Hstack_repr Hstore). intro Heq; exact (Hblock_sep (eq_sym Heq)).
        exact Hsp_ne_sb. exact Hsp_ne_gb. exact Hcb_ne_sp. }
    { exists env_v. split. exact Henv_load'. simpl. exact Henv_repr. }
    { simpl. exact Hextra_load'. }
    { exists gd_ptr. split; [| split; [| split]]. exact Hgd_load'. simpl. exact Hgd_eq. simpl. apply (global_repr_store_other_block hm m m' _ _ _ sb (uso + 8) (Vlong (Int64.repr 5)) Hglobal_repr Hstore). intro Heq2; exact (Hgb_ne (eq_sym Heq2)). exact Hgb_ne_sb. }
