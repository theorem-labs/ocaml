...
Proof.
  ...
  (* ================================================================ *)
  {
    ...
    repeat (rewrite PTree.gso by (compute; congruence)).
    rewrite PTree.gss; eval_cbn.
    rewrite (sem_cast_long_val_repr _ _ _ _ Hval_repr_arg2); eval_cbn.
    rewrite Hsp1_ptrofs.
    ...
    repeat (rewrite PTree.gso by (compute; congruence)).
    rewrite PTree.gss; eval_cbn.
    rewrite (sem_cast_long_val_repr _ _ _ _ Hval_repr_arg3); eval_cbn.
    rewrite Hsp2_ptrofs.
  ...
  (* ================================================================ *)
  {
    ...
    assert (Hstack_m7 :
      stack_repr hm m7 (arg1 :: arg2 :: arg3 :: skipn slotsize (arg1 :: arg2 :: arg3 :: stk_rest)) sp_b new_sp_ofs).
    {
      ...
      { rewrite Hsp1_ptrofs.
        ...
        { pose proof (Mem.load_store_same _ _ _ _ _ _ Hstore3) as Htmp.
          rewrite (val_repr_load_result hm arg2 cv_arg2 Hval_repr_arg2) in Htmp.
          exact Htmp. }
      ...
      { replace (Ptrofs.add (Ptrofs.add new_sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))
        ...
        { pose proof (Mem.load_store_same _ _ _ _ _ _ Hstore4) as Htmp.
          rewrite (val_repr_load_result hm arg3 cv_arg3 Hval_repr_arg3) in Htmp.
          exact Htmp. }
      ...
      rewrite <- Hstk.
      apply (stack_repr_store_other_block hm m6 m7 _ sp_b _ sb (Ptrofs.unsigned so + 32) new_ea_v).
      { apply (stack_repr_store_other_block hm m5 m6 _ sp_b _ sb (Ptrofs.unsigned so + 24) new_env_v).
        { apply (stack_repr_store_other_block hm m4 m5 _ sp_b _ sb (Ptrofs.unsigned so + 0) new_pc_v).
          { apply (stack_repr_store_same_block_lower hm m3 m4 _ sp_b _
                     (Ptrofs.unsigned new_sp_ofs + 16) cv_arg3).
            { apply (stack_repr_store_same_block_lower hm m2 m3 _ sp_b _
              ...
              { apply (stack_repr_store_same_block_lower hm m1 m2 _ sp_b _
                ...
                { apply (stack_repr_store_other_block hm m m1 _ sp_b
                           (Ptrofs.add sp_ofs (Ptrofs.repr (Z.of_nat slotsize * 8)))
                           sb (Ptrofs.unsigned so + 16) (Vptr sp_b new_sp_ofs)).
                  { exact (stack_repr_skipn slotsize hm m (Machine.stack s) sp_b sp_ofs
                             (eq_ind _ (fun stk => stack_repr hm m stk sp_b sp_ofs) Hstack_repr _ (eq_sym Hstk))). }
                  { exact Hstore1. }
