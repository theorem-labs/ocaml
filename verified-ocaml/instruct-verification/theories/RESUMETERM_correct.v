...
Lemma interp_state_pc_co_RT : exists co,
...
Lemma ptrofs_add_zero_RT : forall ofs,
...
Lemma sem_cast_ptr_tint_to_ptr_tint_RT : forall b ofs m,
...
Proof.
  ...
  destruct interp_state_pc_co_RT as [co_is [Hco Hpc_offset]].
  ...
  - (* Part 1: exec_stmt -- the C body executes and produces le', m' *)
    ...
    rewrite sem_cast_ptr_tint_to_ptr_tint_RT; eval_cbn.
    (* Store to s->pc *)
    rewrite Mptr_Mint64.
    rewrite (ptrofs_add_zero_RT so).
  ...
  - (* Part 2: abs_rel on post-state (s <|pc := pc s + 1|>) *)
    ...
    + (* pc_rel *)
      ...
      f_equal.
      (* new_co + (pc+1) * sct = new_pc_ofs - (pc+1)*sct + (pc+1)*sct = new_pc_ofs *)
