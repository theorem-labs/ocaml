...
Proof.
  intro t.
  intros e le m s.
  unfold handle_MAKEBLOCK3. simpl.

  (* Destruct the stack *)
  destruct (Machine.stack s) as [| v1 stk1] eqn:Hstk.
  - (* Empty stack: Error case *)
    exact I.
  - destruct stk1 as [| v2 rest] eqn:Hstk1.
    + (* Single element stack: Error case *)
      exact I.
    + (* v1 :: v2 :: rest: Step case *)
      ...
      (* Invert stack_repr to get head elements *)
      rewrite Hstk in Hstack_repr.
      inversion Hstack_repr as [| v1' stk1' sp_b' sp_ofs' cv1 Hsp0_load Hcv1_repr Hstack_repr_1].
      subst.
      inversion Hstack_repr_1 as [| v2' rest' sp_b'' sp_ofs'' cv2 Hsp1_load Hcv2_repr Hstack_repr_rest].
      subst.
      ...
      destruct interp_state_co_pc_accu_sp_mk3 as [co_is [Hco [Hpc_offset [Haccu_offset Hsp_offset]]]].
      ...
      { subst tag_uchar tag_int. apply int_zero_ext_8_small_mk3. exact Ht_range. }

      assert (Htag_unsigned : Int.unsigned tag_uchar = tag_z).
      { rewrite Htag_uchar_eq. subst tag_int tag_z. apply int_unsigned_repr_small_mk3. exact Ht_range. }
      ...
      (* stack[1] load survives: m -> m1 -> m_alloc *)
      assert (Hsp1_load_m1 : Mem.load Mint64 m1 sp_b (Ptrofs.unsigned (Ptrofs.add sp_ofs (Ptrofs.repr 8))) = Some cv2).
      { erewrite Mem.load_store_other.
        - exact Hsp1_load.
        - exact Hstore_pc.
        - left. exact (Hsp_ne_sb). }
      assert (Hsp1_load_alloc : Mem.load Mint64 m_alloc sp_b (Ptrofs.unsigned (Ptrofs.add sp_ofs (Ptrofs.repr 8))) = Some cv2).
      { apply Halloc_load_pres.
        - exact Hsp1_load_m1.
        - intro Heq; symmetry in Heq; exact (Hnew_ne_sp Heq). }
      ...
      (* stack[1] in m_f0 *)
      assert (Hsp1_load_f0 : Mem.load Mint64 m_f0 sp_b (Ptrofs.unsigned (Ptrofs.add sp_ofs (Ptrofs.repr 8))) = Some cv2).
      { apply Hf0_load_pres.
        - intro Heq; symmetry in Heq; exact (Hnew_ne_sp Heq).
        - exact Hsp1_load_alloc. }

      (* --- Store field 1: *(block + 1) = cv1 --- *)
      destruct (Hcan_store_f1 cv1) as [m_f1 [Hstore_f1 [Hload_f1 [Hf1_load_pres Hcan_store_f2]]]].
      ...
      (* stack[1] in m_f1 *)
      assert (Hsp1_load_f1 : Mem.load Mint64 m_f1 sp_b (Ptrofs.unsigned (Ptrofs.add sp_ofs (Ptrofs.repr 8))) = Some cv2).
      { apply Hf1_load_pres.
        - intro Heq; symmetry in Heq; exact (Hnew_ne_sp Heq).
        - exact Hsp1_load_f0. }

      (* --- Store field 2: *(block + 2) = cv2 --- *)
      destruct (Hcan_store_f2 cv2) as [m_f2 [Hstore_f2 [Hload_f2 [Hf2_load_pres Hf2_perm_pres]]]].

      (* Struct field loads survive field 2 store *)
      assert (Hstruct_preserved_f2 : forall ofs0 v0,
        Mem.load Mint64 m1 sb ofs0 = Some v0 ->
        Mem.load Mint64 m_f2 sb ofs0 = Some v0).
      { intros ofs0 v0 Hld.
        apply Hf2_load_pres.
        - intro Heq; symmetry in Heq; exact (Hnew_ne_sb Heq).
        - apply Hstruct_preserved_f1. exact Hld. }

      (* Sp in m_f2 *)
      assert (Hsp_load_f2 : Mem.load Mint64 m_f2 sb (Ptrofs.unsigned so + 16) = Some (Vptr sp_b sp_ofs)).
      { apply Hstruct_preserved_f2. exact Hsp_load_m1. }

      (* Accu in m_f2 *)
      assert (Haccu_load_f2 : Mem.load Mint64 m_f2 sb (Ptrofs.unsigned so + 8) = Some accu_v).
      { apply Hstruct_preserved_f2. exact Haccu_load_m1. }

      (* --- Store 2: sp field <- Vptr sp_b (sp_ofs + 16) --- *)
      set (new_sp_ofs := Ptrofs.add sp_ofs (Ptrofs.repr 16)).
      ...
      assert (Hsb_writable_f2 :
        Mem.range_perm m_f2 sb (Ptrofs.unsigned so) (Ptrofs.unsigned so + 56) Cur Writable).
      { intros ofs0 Hofs0. eapply Hf2_perm_pres.
      ...
      destruct (store_succeeds_sb m_f2 sb so 16 (Vptr sp_b sp_ofs) Hsb_writable_f2 Hsp_load_f2 ltac:(lia) ltac:(lia) new_sp_v)
        as [m2 Hstore_sp].

      (* Accu in m2 *)
      assert (Haccu_load_m2 : Mem.load Mint64 m2 sb (Ptrofs.unsigned so + 8) = Some accu_v).
      { apply (load_after_store_other m_f2 m2 sb (Ptrofs.unsigned so + 16)
                 (Ptrofs.unsigned so + 8) new_sp_v accu_v Hstore_sp Haccu_load_f2).
        left. lia. }
      ...
      (* Final memory chain: m -> m1 (pc) -> m_alloc (heap_alloc) ->
         m_f0 (field 0) -> m_f1 (field 1) -> m_f2 (field 2) -> m2 (sp) -> m3 (accu) *)
      ...
      set (le2 := PTree.set _t'9 (Vint tag_int) le1).
      set (le3 := PTree.set _tag (Vint tag_uchar) le2).
      set (le4 := PTree.set _t'2 block_v le3).
      set (le5 := PTree.set _block block_v le4).
      set (le6 := PTree.set _t'8 accu_v le5).
      (* After store field 0, read sp for field 1 *)
      set (le7 := PTree.set _t'6 (Vptr sp_b sp_ofs) le6).
      set (le8 := PTree.set _t'7 cv1 le7).
      (* After store field 1, read sp for field 2 *)
      set (le9 := PTree.set _t'4 (Vptr sp_b sp_ofs) le8).
      set (le10 := PTree.set _t'5 cv2 le9).
      (* After store field 2, read sp for sp advance *)
      set (le11 := PTree.set _t'3 (Vptr sp_b sp_ofs) le10).
      set (le' := le11).
      ...
      (* ============================================================== *)
      {
        ...
        { apply (eval_stmt_to_exec clight_ge 10).
          ...
          rewrite (sem_add_pc_1_mk3 cb pc_ofs m); eval_cbn.
          rewrite (sem_cast_ptr_tint_to_ptr_tint_mk3 cb new_pc_ofs); eval_cbn.
          rewrite (ptrofs_add_unsigned so 0 ltac:(lia) ltac:(lia)).
          fold new_pc_v.
          rewrite Hstore_pc; eval_cbn.
          reflexivity. }

        (* Phase 3: Sset _t'9 deref(_t'1) *)
        ...
        (* Phase 4: Sset _tag (cast _t'9 tuchar) *)
        ...
        { apply (eval_stmt_to_exec clight_ge 10).
          ...
          unfold le2.
          rewrite PTree.gss; eval_cbn.
          rewrite (sem_cast_int_to_tuchar_mk3 tag_int m1); eval_cbn.
          reflexivity. }
        ...
        { assert (Hcast_tag : sem_cast (Vint tag_uchar) tuchar tlong m1 =
          ...
          { apply sem_cast_tuchar_to_tlong_mk3. }
          ...
          eapply exec_Scall with
            ...
            (vargs := Vptr sb so :: Vlong (Int64.repr 3) :: Vlong (Int64.repr tag_z) :: nil)
            (f := heap_alloc_fundef)
        ...
        (* Phase 8: Sassign deref(block+0) = _t'8 -- store field 0 *)
        ...
        (* Phase 10: Sset _t'7 deref(_t'6 + 0) -- read stack[0] *)
        ...
        (* Phase 11: Sassign deref(block+1) = _t'7 -- store field 1 *)
        ...
        { apply (eval_stmt_to_exec clight_ge 10).
          ...
          rewrite (sem_cast_long_val_repr hm v1 cv1 m_f0 Hcv1_repr);
            eval_cbn.
          rewrite Hstore_f1; eval_cbn.
          reflexivity. }

        (* Phase 12: Sset _t'4 (s->sp) -- read sp again for field 2 *)
        ...
        (* Phase 13: Sset _t'5 deref(_t'4 + 1) -- read stack[1] *)
        assert (Hexec_read_sp1 : exec_stmt function_entry1 clight_ge e le9 m_f1
            (Sset _t'5
              (Ederef
                (Ebinop Oadd (Etempvar _t'4 (tptr tlong))
                  (Econst_int (Int.repr 1) tint) (tptr tlong)) tlong))
            E0 le10 m_f1 Out_normal).
        { apply (eval_stmt_to_exec clight_ge 10).
          eval_cbn.
          unfold le9.
          rewrite PTree.gss; eval_cbn.
          rewrite (sem_add_sp_1 sp_b sp_ofs m_f1); eval_cbn.
          rewrite Hsp1_load_f1; eval_cbn.
          reflexivity. }

        (* Phase 14: Sassign deref(block+2) = _t'5 -- store field 2 *)
        assert (Hexec_store_f2 : exec_stmt function_entry1 clight_ge e le10 m_f1
            (Sassign
              (Ederef
                (Ebinop Oadd (Ecast (Etempvar _block tlong) (tptr tlong))
                  (Econst_int (Int.repr 2) tint) (tptr tlong)) tlong)
              (Etempvar _t'5 tlong))
            E0 le10 m_f2 Out_normal).
        { apply (eval_stmt_to_exec clight_ge 10).
          eval_cbn.
          unfold le10.
          rewrite PTree.gso by (compute; congruence).
          ...
          rewrite PTree.gss; eval_cbn.
          fold block_v.
          replace (sem_cast block_v tlong (tptr tlong) m_f1) with (Some block_v) by (unfold block_v, sem_cast; simpl classify_cast; reflexivity); eval_cbn.
          unfold block_v at 1. rewrite (sem_add_sp_2 new_b new_ofs m_f1); eval_cbn.
          unfold le10. rewrite PTree.gss; eval_cbn.
          rewrite (sem_cast_long_val_repr hm v2 cv2 m_f1 Hcv2_repr);
            eval_cbn.
          rewrite Hstore_f2; eval_cbn.
          reflexivity. }

        (* Phase 15: Sset _t'3 (s->sp) -- read sp for sp advance *)
        assert (Hexec_read_sp3 : exec_stmt function_entry1 clight_ge e le10 m_f2
            (Sset _t'3
              (Efield (Ederef (Etempvar _s (tptr (Tstruct _interp_state noattr)))
                       (Tstruct _interp_state noattr)) _sp (tptr tlong)))
            E0 le11 m_f2 Out_normal).
        { apply (eval_stmt_to_exec clight_ge 10).
          eval_cbn.
          unfold le10.
          rewrite PTree.gso by (compute; congruence).
          ...
          rewrite Mptr_Mint64; eval_cbn.
          rewrite (ptrofs_add_unsigned so 16 ltac:(lia) ltac:(lia)).
          rewrite Hsp_load_f2; eval_cbn.
          reflexivity. }

        (* Phase 16: Sassign (s->sp) (_t'3 + 2) -- sp advance by 2 *)
        assert (Hexec_store_sp : exec_stmt function_entry1 clight_ge e le11 m_f2
            (Sassign
              (Efield (Ederef (Etempvar _s (tptr (Tstruct _interp_state noattr)))
                       (Tstruct _interp_state noattr)) _sp (tptr tlong))
              (Ebinop Oadd (Etempvar _t'3 (tptr tlong))
                (Econst_int (Int.repr 2) tint) (tptr tlong)))
            E0 le11 m2 Out_normal).
        { apply (eval_stmt_to_exec clight_ge 10).
          eval_cbn.
          unfold le11.
          rewrite PTree.gso by (compute; congruence).
          ...
          rewrite Hco; eval_cbn.
          rewrite Hsp_offset; eval_cbn.
          unfold le11. rewrite PTree.gss; eval_cbn.
          rewrite (sem_add_sp_2 sp_b sp_ofs m_f2); eval_cbn.
          rewrite Mptr_Mint64; eval_cbn.
          rewrite (sem_cast_ptr_to_ptr sp_b (Ptrofs.add sp_ofs (Ptrofs.repr 16)) m_f2); eval_cbn.
          rewrite (ptrofs_add_unsigned so 16 ltac:(lia) ltac:(lia)).
          unfold new_sp_v, new_sp_ofs in Hstore_sp.
          rewrite Hstore_sp; eval_cbn.
          reflexivity. }

        (* Phase 17: Sassign (s->accu) _block *)
        assert (Hexec_store_accu : exec_stmt function_entry1 clight_ge e le11 m2
            (Sassign
              (Efield (Ederef (Etempvar _s (tptr (Tstruct _interp_state noattr)))
                       (Tstruct _interp_state noattr)) _accu tlong)
              (Etempvar _block tlong))
            E0 le11 m3 Out_normal).
        { apply (eval_stmt_to_exec clight_ge 10).
          eval_cbn.
          unfold le11.
          rewrite PTree.gso by (compute; congruence).
        ...
        (* Phase 18: Sreturn 0 *)
        assert (Hexec_return : exec_stmt function_entry1 clight_ge e le11 m3
            (Sreturn (Some (Econst_int (Int.repr 0) tint)))
            E0 le11 m3 (Out_return (Some (Vint (Int.repr 0), tint)))).
        ...
        (* Seq: Sset _t'8 (accu); Sassign *(block+0) *)
        ...
        (* Seq: Sset _t'6 (sp); Sset _t'7 deref(t6+0); Sassign *(block+1) *)
        ...
        (* Seq: Sset _t'4 (sp); Sset _t'5 deref(t4+1); Sassign *(block+2) *)
        assert (Hexec_read_sp_store_f2 :
        ...
        { replace E0 with (E0 ** E0) by reflexivity.
          eapply exec_Sseq_1.
          - exact Hexec_read_sp2.
          - replace E0 with (E0 ** E0) by reflexivity.
            eapply exec_Sseq_1; eauto. }

        (* Seq: Sset _t'3 (sp); Sassign s->sp = t3+2 *)
        assert (Hexec_sp_advance :
          exec_stmt function_entry1 clight_ge e le10 m_f2
            (Ssequence
              (Sset _t'3
                (Efield (Ederef (Etempvar _s (tptr (Tstruct _interp_state noattr)))
                         (Tstruct _interp_state noattr)) _sp (tptr tlong)))
        ...
        (* Seq: read_store_f0; read_sp_store_f1; read_sp_store_f2; sp_accu *)
        ...
        change (fn_body f_instr_MAKEBLOCK3) with
          (Ssequence
            (Ssequence
              (Ssequence
                (Ssequence
      ...
      (* ============================================================== *)
      {
        ...
        (* pc field at uso+0: written by store_pc in m1, preserved through
           m_alloc, m_f0, m_f1, m_f2 (stores to new_b), m2 (store at uso+16),
           m3 (store at uso+8) *)
        ...
        { pose proof (load_after_store_same m m1 sb (uso + 0) new_pc_v Hstore_pc) as Htmp.
          unfold new_pc_v in Htmp |- *. rewrite load_result_vptr_mk3 in Htmp. exact Htmp. }

        assert (Hpc_load_f2 : Mem.load Mint64 m_f2 sb (uso + 0) = Some new_pc_v).
        { apply Hstruct_preserved_f2. exact Hpc_load_m1. }

        assert (Hpc_load_m2 : Mem.load Mint64 m2 sb (uso + 0) = Some new_pc_v).
        { apply (load_after_store_other m_f2 m2 sb (uso + 16) (uso + 0)
                   new_sp_v new_pc_v Hstore_sp Hpc_load_f2). left. lia. }
        ...
        { pose proof (load_after_store_same m2 m3 sb (uso + 8) block_v Hstore_accu) as Htmp.
          unfold block_v in Htmp |- *. rewrite load_result_vptr_mk3 in Htmp. exact Htmp. }
        ...
        { pose proof (load_after_store_same m_f2 m2 sb (uso + 16) new_sp_v Hstore_sp) as Htmp.
          unfold new_sp_v in Htmp |- *. rewrite load_result_vptr_mk3 in Htmp. exact Htmp. }
        ...
        { intros fo v Hfo Hload.
          ...
          assert (H2 : Mem.load Mint64 m_f2 sb (uso + fo) = Some v).
          { apply Hstruct_preserved_f2. exact H1. }
          assert (H3 : Mem.load Mint64 m2 sb (uso + fo) = Some v).
          { apply (load_after_store_other m_f2 m2 sb (uso + 16) (uso + fo)
        ...
        (* Stack repr in m3: the new stack is 'rest' at sp_b (sp_ofs + 16) *)
        ...
        (* Hstack_repr_rest : stack_repr hm m rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8)) *)

        (* Thread through memories *)
        assert (Hstack_rest_m1 : stack_repr hm m1 rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        { eapply stack_repr_store_other_block; eauto. }

        assert (Hstack_rest_alloc : stack_repr hm m_alloc rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        ...
        assert (Hstack_rest_f0 : stack_repr hm m_f0 rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        { eapply stack_repr_store_other_block; eauto. }

        assert (Hstack_rest_f1 : stack_repr hm m_f1 rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        { eapply stack_repr_store_other_block; eauto. }

        assert (Hstack_rest_f2 : stack_repr hm m_f2 rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        { eapply stack_repr_store_other_block; eauto. }

        assert (Hstack_rest_m2 : stack_repr hm m2 rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        { eapply stack_repr_store_other_block; eauto. }

        assert (Hstack_rest_m3 : stack_repr hm m3 rest sp_b (Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8))).
        { eapply stack_repr_store_other_block; eauto. }

        (* We need: new_sp_ofs = Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8) *)
        assert (Hnew_sp_eq : new_sp_ofs = Ptrofs.add (Ptrofs.add sp_ofs (Ptrofs.repr 8)) (Ptrofs.repr 8)).
        { unfold new_sp_ofs.
          rewrite Ptrofs.add_assoc. reflexivity. }

        assert (Hstack_rest_m3' : stack_repr hm m3 rest sp_b new_sp_ofs).
        { rewrite Hnew_sp_eq. exact Hstack_rest_m3. }
        ...
        assert (Hglobal_f2 : global_repr hm m_f2 (Machine.global s) gb go0).
        { eapply global_repr_store_other_block; eauto. }
        ...
        (* 1. _s in le' *)
        { subst le' le11 le10 le9 le8 le7 le6 le5 le4 le3 le2 le1.
        ...
        (* 4. sp field -- new_sp_v = Vptr sp_b (sp_ofs + 16) *)
          ...
          - split.
            + (* sp_writable *)
              ...
              * eapply Mem.perm_store_1. exact Hstore_pc.
                ...
                rewrite (Ptrofs.unsigned_repr 16) in Hofs0.
                2: { unfold Ptrofs.max_unsigned. pose proof Ptrofs.modulus_pos. lia. }
            ...
            + (* sp_align *)
              ...
              exists (k + 2). rewrite Ptrofs.add_unsigned.
              rewrite (Ptrofs.unsigned_repr 16).
              2: { unfold Ptrofs.max_unsigned. pose proof Ptrofs.modulus_pos. lia. }
              rewrite Ptrofs.unsigned_repr.
              2: { pose proof (Ptrofs.unsigned_range sp_ofs).
        ...
        { exists (Vptr gb go0). split; [| split; [| split]].
          - exact Hgd_load3.
          - simpl. reflexivity.
          - simpl. apply Hglobal_repr_ext. exact Hglobal3.
          - exact Hgb_ne_sb. }
