(* PUSHENVACC3_correct.v -- PUSHENVACC3 = PUSH then ENVACC2.

   C handler (f_instr_PUSHENVACC3):
     t5 = s->sp; t1 = (long ptr)(t5 - 1); s->sp = t1;
     t4 = s->accu; *t1 = t4;
     t2 = s->env; t3 = *((long ptr)t2 + 2); s->accu = t3; return 0;

   Rocq (handle_PUSHENVACC 3):
     let new_stack := accu :: stack in
     match field_or_heap s s.(env) 3 with
...
Lemma sem_cast_long_to_ptr_vptr_PEA3 : forall b ofs m,
  sem_cast (Vptr b ofs) tlong (tptr tlong) m = Some (Vptr b ofs).
Proof.
  intros. unfold sem_cast. simpl classify_cast. reflexivity.
Qed.

Lemma sem_add_ptr_long_PEA3 : forall b ofs m,
...
Definition pushenvacc_step_pre_3 (e : Clight.env) (m : mem) (s : Machine.state)
  ...
  exists sp_b sp_ofs,
    ...
    (forall v,
      field_or_heap s s.(Machine.env) 3 = Some v ->
      forall env_v,
...
Proof.
  ...
  2: { reflexivity. }
  {
    ...
    unfold pushenvacc_step_pre_3 in Hstep_pre. fold sb so hm in Hstep_pre.
    ...
    assert (Hfield_load_m1 : Mem.load Mint64 m1 env_b (Ptrofs.unsigned (Ptrofs.add env_ofs (Ptrofs.repr 24))) = Some cv).
    { erewrite Mem.load_store_other. exact Hfield_load. exact Hstore_sp.
      left. exact Henv_ne_sb. }
    assert (Hfield_load_m2 : Mem.load Mint64 m2 env_b (Ptrofs.unsigned (Ptrofs.add env_ofs (Ptrofs.repr 24))) = Some cv).
...

    {
      ...
      rewrite PTree.gss; eval_cbn.
      rewrite sem_cast_long_to_ptr_vptr_PEA3; eval_cbn.
      rewrite (sem_add_ptr_long_PEA3 env_b env_ofs m2); eval_cbn.
