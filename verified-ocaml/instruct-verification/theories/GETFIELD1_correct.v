...
(* The precondition for the Step case: when field_or_heap succeeds at
   index 1, the C memory must contain the corresponding value at the
   pointer location offset by 8 bytes (1 * sizeof(long)), and the
   accu must be representable as a Vptr. *)
Definition heap_field_loadable_1
...
Proof.
  ...
  }
  {
    ...
    unfold heap_field_loadable_1 in Hhfl.
