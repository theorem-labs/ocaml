    ...
    handler_correct (handle_MAKEFLOATBLOCK n) f_instr_MAKEFLOATBLOCK
      (fun e m s ard =>
         ...
         (forall le_pre m_field0 new_b new_ofs sp_b sp_ofs,
            ...
            stack_repr hm m_field0 (Machine.stack s) sp_b sp_ofs ->
            ...
            exists le_loop m_loop sp_ofs_loop,
              ...
              stack_repr hm m_loop (skipn (Nat.sub n 1) (Machine.stack s))
                sp_b sp_ofs_loop /\
...
Proof.
  ...
  assert (Hstack_field0 : stack_repr hm m_field0 (Machine.stack s) sp_b sp_ofs).
  { eapply stack_repr_store_other_block; eauto. }
    ...
    handler_correct (handle_MAKEFLOATBLOCK n) f_instr_MAKEFLOATBLOCK
      (fun e m s ard =>
         ...
         (forall le_pre m_alloc0 new_b new_ofs sp_b sp_ofs,
            ...
            val_repr hm (Machine.accu s) (Vptr sp_b sp_ofs) ->
            False) /\
         ...
         (forall le_pre m_alloc0 new_b new_ofs sp_b sp_ofs accu_v0,
            ...
            val_repr hm (Machine.accu s) accu_v0 ->
            Mem.load Mint64 m_alloc0 sb (Ptrofs.unsigned so + 16) = Some (Vptr sp_b sp_ofs) ->
            stack_repr hm m_alloc0 (Machine.stack s) sp_b sp_ofs ->
            ...
            exists le_out m_out sp_ofs_out,
              ...
              stack_repr hm m_out (skipn (Nat.sub n 1) (Machine.stack s))
                sp_b sp_ofs_out /\
...
Proof.
  ...
  destruct Hstep_pre as (He_heap_alloc & Hcode_size &
    Hsize_range & Hhm_fresh &
    Hgb_valid & [b_ha [Hfind_symbol Hfind_funct]] &
    Halloc_spec_all & _Hdummy & Hbody_post).
  ...
  (* --- Temp environment setup (up to Sset _block) --- *)
  set (le1 := PTree.set _t'1 (Vptr cb pc_ofs) le).
  set (le2 := PTree.set _t'9 (Vint size_int) le1).
  set (le3 := PTree.set _size (Vlong (Int64.repr (Int.signed size_int))) le2).
  set (block_v := Vptr new_b new_ofs).
  ...
  assert (Hle5_block : le5 ! _block = Some block_v).
  { subst le5. rewrite PTree.gss. reflexivity. }

  assert (Hle5_size : le5 ! _size = Some (Vlong (Int64.repr (Z.of_nat n)))).
  { subst le5 le4.
    repeat (rewrite PTree.gso by (compute; congruence)).
    subst le3. rewrite PTree.gss. f_equal. f_equal. exact Hsize_eq. }
  ...
  (* --- Invoke body postcondition --- *)
  assert (Halloc_load_pres_refl : forall b ofs chunk v,
    Mem.load chunk m_alloc b ofs = Some v -> b <> new_b ->
    Mem.load chunk m_alloc b ofs = Some v).
  { intros; assumption. }

  destruct (Hbody_post le5 m_alloc new_b new_ofs sp_b sp_ofs accu_v
              Hle5_s Hle5_block Hle5_size
              Hnew_ne_sb Hnew_ne_sp Hnew_ne_gb Hnew_ne_cb
              Haccu_load_alloc Haccu_repr
              Hsp_load_alloc Hstack_alloc Hsb_writable_alloc
              Halloc_load_pres_refl)
    as [le_out [m_out [sp_ofs_out
         (Hexec_body & Hle_out_s & Hle_out_block &
          Hpc_preserved & Haccu_out & Hsp_out &
          Hstack_out & Hsp_ofs_out_ge8 & Hsp_align_out & Hsp_rep_out &
          Hsp_range_containment &
          Hfield_survive & Hperm_pres & Hgb_pres)]]].

  (* --- Build witnesses --- *)
  exists le_out. exists m_out.
  exists (Out_return (Some (Vint (Int.repr 0), tint))).
  ...
  (* ============================================================== *)
  {
    ...
    { eapply exec_Scall with
        ...
        (cconv := cc_default)
        (vf := Vptr b_ha Ptrofs.zero)
        (vargs := Vptr sb so :: Vlong (Int64.repr (Z.of_nat n)) :: Vlong (Int64.repr 254) :: nil)
        (f := heap_alloc_fundef)
        (vres := Vptr new_b new_ofs).
      - reflexivity.
      - eapply eval_Elvalue.
        ...
        + econstructor.
          * eapply eval_Ebinop.
            ...
            -- eapply eval_Ebinop.
               ...
               ++ simpl. reflexivity.
            -- simpl.
               change (Ptrofs.to_int64 (Ptrofs.repr 8)) with (Int64.repr 8).
               change (Int64.divu (Int64.repr 8) (Int64.repr 8)) with (Int64.repr 1).
               unfold sem_mul, sem_binarith.
               change (classify_binarith tulong tulong) with (bin_case_l Unsigned).
               change (binarith_type (bin_case_l Unsigned)) with tulong.
               rewrite (sem_cast_ulong_to_tulong_mfb (Int64.repr (Int.signed size_int)) m1).
               rewrite (sem_cast_ulong_to_tulong_mfb (Int64.repr 1) m1).
               simpl. rewrite Int64.mul_one. rewrite Hsize_eq.
               reflexivity.
          * apply sem_cast_ulong_to_tlong_mfb.
          * econstructor.
            -- econstructor.
            -- rewrite sem_cast_254_to_tlong. rewrite int_signed_254. reflexivity.
            -- constructor.
      - exact Hfind_funct.
      - reflexivity.
      - eapply eval_funcall_external. exact Hext_call.
    ...
    (* Phase 7: body after setblock -- from postcondition *)
    (* Hexec_body covers the rest *)

    (* Phase 8: return 0 *)
    assert (Hexec_return : exec_stmt function_entry1 clight_ge e le_out m_out
        (Sreturn (Some (Econst_int (Int.repr 0) tint)))
        E0 le_out m_out (Out_return (Some (Vint (Int.repr 0), tint)))).
    ...
    (* Combine: S5-body (alloc + block + body_after) *)
    assert (Hexec_alloc_to_end :
      exec_stmt function_entry1 clight_ge e le3 m1
        (Ssequence
          (Ssequence
            (Scall (Some _t'2)
    ...
    (* Combine: S1-S4 + S5-body = full pre-return body *)
    ...
    change (fn_body f_instr_MAKEFLOATBLOCK) with
      (Ssequence
        (Ssequence
          (Ssequence
            (Ssequence
  ...
  (* ============================================================== *)
  {
    ...
    set (hm' := fun a => if Nat.eqb a addr then Some (new_b, new_ofs) else hm a).
    ...
    (* pc field: written in m1 as new_pc_v, preserved through alloc and body *)
    ...
    { pose proof (load_after_store_same m m1 sb (uso + 0) new_pc_v Hstore_pc) as Htmp.
      unfold new_pc_v in Htmp |- *. rewrite load_result_vptr_mfb in Htmp. exact Htmp. }

    assert (Hpc_load_alloc : Mem.load Mint64 m_alloc sb (uso + 0) = Some new_pc_v).
    { apply Hstruct_preserved. exact Hpc_load_m1. }

    assert (Hpc_load_out : Mem.load Mint64 m_out sb (uso + 0) = Some new_pc_v).
    { apply Hpc_preserved. exact Hpc_load_alloc. }

    (* accu field: stored by body postcondition *)
    (* Haccu_out already gives us this *)

    (* Fields at offsets >= 24 survive m -> m1 -> m_alloc -> m_out *)
    assert (Hfield_chain : forall fo v,
      fo >= 24 ->
      Mem.load Mint64 m sb (uso + fo) = Some v ->
      Mem.load Mint64 m_out sb (uso + fo) = Some v).
    { intros fo v Hfo Hload.
      assert (H1 : Mem.load Mint64 m1 sb (uso + fo) = Some v).
      { apply (load_after_store_other m m1 sb (uso + 0) (uso + fo) new_pc_v v Hstore_pc Hload). right. lia. }
      assert (H2 : Mem.load Mint64 m_alloc sb (uso + fo) = Some v).
      { apply Hstruct_preserved. exact H1. }
      apply Hfield_survive. exact Hfo. exact H2. }

    assert (Henv_load_out : Mem.load Mint64 m_out sb (uso + 24) = Some env_v).
    { apply Hfield_chain; [lia | exact Henv_load]. }

    assert (Hextra_load_out : Mem.load Mint64 m_out sb (uso + 32) =
              Some (Vlong (Int64.repr (Z.of_nat (extra_args s))))).
    { apply Hfield_chain; [lia | exact Hextra_load]. }

    assert (Hgd_load_out : Mem.load Mint64 m_out sb (uso + 40) = Some (Vptr gb go0)).
    { apply Hfield_chain; [lia | exact Hgd_load]. }

    assert (Hts_load_out : Mem.load Mint64 m_out sb (uso + 48) = Some ts_ptr).
    { apply Hfield_chain; [lia | exact Hts_load]. }

    (* Global repr in m_out *)
    ...
    assert (Hglobal_out : global_repr hm m_out (Machine.global s) gb go0).
    { clear -Hglobal_alloc Hgb_pres.
      induction Hglobal_alloc.
      - constructor.
      - econstructor; eauto. }
    ...
    (* 4. sp field -- updated by body *)
    { exists (Vptr sp_b sp_ofs_out), sp_b, sp_ofs_out.
      split; [| split; [| split; [| split; [| split; [| split; [| split; [| split]]]]]]].
      - exact Hsp_out.
      - reflexivity.
      - simpl. apply Hstack_repr_ext. exact Hstack_out.
      - exact Hsp_ne_sb.
      - exact Hsp_ne_gb.
      - exact Hcb_ne_sp.
      - exact Hsp_ofs_out_ge8.
      - exact Hsp_rep_out.
      - split.
        + intros ofs0 Hofs0.
          apply Hperm_pres.
    ...
    (* 6. extra_args field *)
    { simpl. exact Hextra_load_out. }

    (* 7. global_data field *)
    { exists (Vptr gb go0). split; [| split; [| split]].
      - exact Hgd_load_out.
      - simpl. reflexivity.
      - simpl. apply Hglobal_repr_ext. exact Hglobal_out.
      - exact Hgb_ne_sb. }
