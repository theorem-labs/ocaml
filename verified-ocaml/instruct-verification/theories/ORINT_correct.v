...
Proof.
  ...
  set (result_v := Vlong (Int64.or (Int64.repr (a * 2 + 1)) (Int64.repr (b * 2 + 1)))).
  ...
  { apply (eval_stmt_to_exec clight_ge 10). eval_cbn.
    ...
    (* -- ORINT-specific: evaluate (Ecast (Ebinop Oor ...)) -- *)
    ...
    rewrite sem_cast_long_vlong; eval_cbn.
    rewrite sem_or_long_long; eval_cbn.
    rewrite sem_cast_long_vlong; eval_cbn.
    rewrite sem_cast_long_vlong; eval_cbn.
    rewrite (ptrofs_add_unsigned so 8 ltac:(lia) ltac:(lia)).
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    { exists result_v. split. exact Haccu_load'. simpl. unfold result_v. rewrite tagged_orint_arith. constructor. }
