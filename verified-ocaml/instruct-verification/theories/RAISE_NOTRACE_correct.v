(* RAISE_NOTRACE_correct.v -- RAISE_NOTRACE handler correctness proof.

   RAISE_NOTRACE pops a trap frame from the stack, restoring pc, sp,
   trap_sp, env, and extra_args.  This is the exception-raising
   instruction without backtrace recording.  The C body is identical
   to RAISE; the Rocq model reuses do_raise.
   ...
   KNOWN LIMITATION: Same Vptr/Vlong gap as PUSHTRAP and RAISE.
...
(* ================================================================== *)
(* Step precondition for RAISE_NOTRACE                                 *)
(* ================================================================== *)
...
Definition raise_notrace_step_pre
...
Lemma skipn_cons_eq_notrace : forall {A : Type} (k : nat) (l : list A) (x : A) (xs : list A),
  skipn k l = x :: xs ->
  skipn (S k) l = xs.
Proof.
  intros A k. revert k.
              ...
              ** destruct frame3 as [| v3 rest].
                 ...
                 --- destruct v3 as [saved_ea | | |].
                     +++ (* Val_int saved_ea -- Step case *)
                         ...
                         unfold raise_notrace_step_pre in Hstep_pre.
