...
Proof.
  ...
  destruct (store_succeeds_from_load m sb (Ptrofs.unsigned so + 8) accu_v cv3 Haccu_load) as [m' Hstore].
  ...
  { apply (eval_stmt_to_exec clight_ge 10). eval_cbn.
    ...
    rewrite (sem_cast_long_val_repr _ _ _ _ Hval_repr3); eval_cbn.
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    assert (Haccu_load' : Mem.load Mint64 m' sb (uso + 8) = Some cv3). { pose proof (load_after_store_same m m' sb (uso + 8) cv3 Hstore) as Htmp. rewrite (val_repr_load_result hm v3 cv3 Hval_repr3) in Htmp. exact Htmp. }
    ...
    { exists (Vptr sp_b sp_ofs), sp_b, sp_ofs. split; [| split; [| split; [| split; [| split; [| split; [| split; [| split]]]]]]]. exact Hsp_load'. reflexivity. simpl. rewrite Hstk.
            apply (stack_repr_store_other_block hm m m' _ sp_b sp_ofs sb (uso + 8) cv3 Hstack_repr Hstore). intro Heq; exact (Hsp_ne_sb (eq_sym Heq)).
        - exact Hsp_ne_sb.
        - exact Hsp_ne_gb.
        - exact Hcb_ne_sp.
        - exact Hsp_ge8.
    ...
    { exists gd_ptr. split; [| split; [| split]]. exact Hgd_load'. simpl. exact Hgd_eq. simpl. apply (global_repr_store_other_block hm m m' _ _ _ sb (uso + 8) cv3 Hglobal_repr Hstore). intro Heq2; exact (Hgb_ne (eq_sym Heq2)). exact Hgb_ne_sb. }
