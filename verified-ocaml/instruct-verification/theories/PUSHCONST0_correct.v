...
Proof.
  ...
  (* Store 3: accu field <- Vlong (Int64.repr 1) *)
  ...
  destruct (store_succeeds_from_load m2 sb (Ptrofs.unsigned so + 8)
              accu_v (Vlong (Int64.repr 1)) Haccu_load_m2) as [m3 Hstore_const].
  ...
  (* ============================================================== *)
  {
    ...
    (* Statement 6: Sassign (s->accu) ((0 << 1) + 1) *)
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    rewrite Hle_s; eval_cbn.

    (* rvalue: ((cast 0 tlong) << 1) + 1 *)
  ...
  (* ============================================================== *)
  {
    ...
    (* Helper: loads on sb survive store 3 (same block, different offset) *)
    (* Store 3 is at uso + 8, so any load at ofs <> uso+8 survives *)
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 0)
               (Vlong (Int64.repr 1)) pc_ptr Hstore_const).
      - apply Hload_sb_m2.
    ...
    assert (Haccu_load3 : Mem.load Mint64 m3 sb (uso + 8) = Some (Vlong (Int64.repr 1))).
    { pose proof (load_after_store_same m2 m3 sb (uso + 8)
                    (Vlong (Int64.repr 1)) Hstore_const) as Htmp.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 16)
               (Vlong (Int64.repr 1)) (Vptr sp_b new_sp_ofs) Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 24)
               (Vlong (Int64.repr 1)) env_v Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 32)
               (Vlong (Int64.repr 1)) _ Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 40)
               (Vlong (Int64.repr 1)) gd_ptr Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 48)
               (Vlong (Int64.repr 1)) ts_ptr Hstore_const).
      - apply Hload_sb_m2.
    ...
    { exists (Vptr sp_b new_sp_ofs), sp_b, new_sp_ofs.
      ...
      - reflexivity.
      - simpl.
        (* stack_repr must survive all 3 stores.
           Store 1: sb at uso+16 (different block from sp_b)
           Store 2: sp_b at new_sp_ofs (the cons store)
           Store 3: sb at uso+8 (different block from sp_b)
           We build up: stack_repr in m1 (after store 1), then
           cons after store 2, then survive store 3 (different block). *)
    ...
    { exists gd_ptr. split; [| split; [| split]].
      ...
      - simpl. exact Hgd_eq.
      - simpl.
        (* global_repr survives all 3 stores (different blocks) *)
        apply (global_repr_store_other_block hm m2 m3 _
                 (ar_global_block ard) (ar_global_ofs ard)
                 sb (uso + 8) (Vlong (Int64.repr 1))).
