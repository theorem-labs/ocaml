...
Definition pushenvacc_step_pre_1 (e : Clight.env) (m : mem) (s : Machine.state)
...
Proof.
  ...
  2: { reflexivity. }
  {
    ...
    (* Extract step_pre *)
    unfold pushenvacc_step_pre_1 in Hstep_pre. fold sb so hm in Hstep_pre.
    ...
    (* Env field load survives stores 1 and 2 *)
    assert (Hfield_load_m1 : Mem.load Mint64 m1 env_b (Ptrofs.unsigned (Ptrofs.add env_ofs (Ptrofs.repr 8))) = Some cv).
    { erewrite Mem.load_store_other. exact Hfield_load. exact Hstore_sp.
      left. exact Henv_ne_sb. }
    assert (Hfield_load_m2 : Mem.load Mint64 m2 env_b (Ptrofs.unsigned (Ptrofs.add env_ofs (Ptrofs.repr 8))) = Some cv).
    { erewrite Mem.load_store_other. exact Hfield_load_m1. exact Hstore_accu.
      left. exact Henv_ne_spb. }

    (* Accu field survived stores 1 and 2 *)
    ...
    (* Env load survived stores 1 and 2 for C execution *)
    ...
    (* ============================================================== *)
    {
      ...
      (* Sset _t'1 (cast (sub _t'5 1) (tptr tlong)) *)
      ...
      (* Sset _t'3 (deref ((cast _t'2 (tptr tlong)) + 1)) *)
      rewrite PTree.gss; eval_cbn.
      rewrite sem_cast_long_to_ptr_vptr_PEA1; eval_cbn.
      rewrite (sem_add_sp_1 env_b env_ofs m2); eval_cbn.
      ...
      try rewrite (sem_cast_long_val_repr _ _ _ _ Hfield_repr); eval_cbn.
      ...
      { exists (Vptr sp_b new_sp_ofs), sp_b, new_sp_ofs.
        ...
        - simpl.
          ...
          apply (stack_repr_store_other_block hm m2 m3
                   ...
                   (uso + 8) cv
                   Hstack_cons_m2 Hstore_env_field).
          intro Heq; exact (Hsp_ne_sb (eq_sym Heq)).
        - exact Hsp_ne_sb.
        - exact Hsp_ne_gb.
        - exact Hcb_ne_sp.
