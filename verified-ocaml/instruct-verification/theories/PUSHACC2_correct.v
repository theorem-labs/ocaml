...
Proof.
  ...
  assert (Hstack_m1 : stack_repr hm cb co m1 (v0 :: v1 :: rest) sp_b sp_ofs).
  ...
  assert (Hstack_m2 : stack_repr hm cb co m2 (Machine.accu s :: v0 :: v1 :: rest) sp_b new_sp_ofs).
  ...
  destruct (store_succeeds_sb m2 sb so 8 accu_v Hsb_writable_m2 Haccu_load_m2 ltac:(lia) ltac:(lia) cxb1)
  ...
  (* ============================================================== *)
  {
    ...
    try rewrite (sem_cast_long_val_repr _ _ _ _ _ _ Hval_b1); eval_cbn.
  ...
  (* ============================================================== *)
  {
    ...
    { pose proof (load_after_store_same m2 m3 sb (uso + 8) cxb1 Hstore_accu_field) as Htmp.
      rewrite (val_repr_load_result hm cb co v1 cxb1 Hval_b1) in Htmp.
      exact Htmp. }
    ...
    { exists (Vptr sp_b new_sp_ofs), sp_b, new_sp_ofs.
      ...
      - simpl length. rewrite Nat2Z.inj_succ. rewrite Hnew_sp_unsigned.
        ...
        replace (Ptrofs.unsigned sp_ofs - 8 + 8 * Z.succ (Z.of_nat (S (S (length rest)))))
          with (Ptrofs.unsigned sp_ofs + 8 * Z.of_nat (S (S (length rest)))) by lia.
        intros ofs' Hofs'.
