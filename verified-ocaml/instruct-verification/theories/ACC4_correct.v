...
Proof.
  ...
  destruct (store_succeeds_from_load m sb (Ptrofs.unsigned so + 8) accu_v cv4 Haccu_load) as [m' Hstore].
  set (le' := PTree.set _t'2 cv4 (PTree.set _t'1 (Vptr sp_b sp_ofs) le)).
  ...
  { apply (eval_stmt_to_exec clight_ge 10). eval_cbn.
    ...
    rewrite sem_add_sp_4; eval_cbn. rewrite Hload_sp4; eval_cbn.
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    assert (Hpc_load' : Mem.load Mint64 m' sb (uso + 0) = Some pc_ptr). { apply (load_after_store_other m m' sb (uso + 8) (uso + 0) cv4 pc_ptr Hstore Hpc_load). left. lia. }
    assert (Hsp_load' : Mem.load Mint64 m' sb (uso + 16) = Some (Vptr sp_b sp_ofs)). { apply (load_after_store_other m m' sb (uso + 8) (uso + 16) cv4 (Vptr sp_b sp_ofs) Hstore Hsp_load). right. lia. }
    assert (Henv_load' : Mem.load Mint64 m' sb (uso + 24) = Some env_v). { apply (load_after_store_other m m' sb (uso + 8) (uso + 24) cv4 env_v Hstore Henv_load). right. lia. }
    ...
    { exists (Vptr sp_b sp_ofs), sp_b, sp_ofs. split; [| split]. exact Hsp_load'. reflexivity. simpl. rewrite Hstk.
      apply (stack_repr_store_other_block hm m m' _ sp_b sp_ofs sb (uso + 8) cv4 Hstack_repr Hstore). intro Heq; exact (Hblock_sep (eq_sym Heq)). }
    { exists env_v. split. exact Henv_load'. simpl. exact Henv_repr. }
    { simpl. exact Hextra_load'. }
    { exists gd_ptr. split; [| split]. exact Hgd_load'. simpl. exact Hgd_eq. simpl. apply (global_repr_store_other_block hm m m' _ _ _ sb (uso + 8) cv4 Hglobal_repr Hstore). intro Heq2; exact (Hgb_ne (eq_sym Heq2)). }
