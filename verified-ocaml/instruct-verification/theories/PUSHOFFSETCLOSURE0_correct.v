...
Proof.
  ...
  - simpl.
    ...
    destruct interp_state_co_env as [co_is2 [Hco2 [Henv_offset _]]].
    ...
    (* env_v survives stores 1 and 2, for use in store 3 and env read *)
    assert (Henv_load_m1 : Mem.load Mint64 m1 sb (Ptrofs.unsigned so + 24) = Some env_v).
    { apply (load_after_store_other m m1 sb (Ptrofs.unsigned so + 16)
               (Ptrofs.unsigned so + 24) (Vptr sp_b new_sp_ofs) env_v
    ...
    (* Rewrite Henv_repr for Val_block case *)
    rewrite Henv_eq in Henv_repr.

    destruct (store_succeeds_sb m2 sb so 8 accu_v Hsb_writable_m2 Haccu_load_m2 ltac:(lia) ltac:(lia) env_v) as [m3 Hstore_env].

    (* Witnesses *)
    set (le' := PTree.set _t'2 env_v
    ...
    (* ============================================================== *)
    {
      ...
      (* Statement 7: Sassign (s->accu) _t'2 -- store env to accu field *)
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
      rewrite PTree.gso by (compute; congruence).
    ...
    (* ============================================================== *)
    {
      ...
      { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 16)
                 env_v (Vptr sp_b new_sp_ofs) Hstore_env).
        - apply Hload_sb_m2.
      ...
      { exists (Vptr sp_b new_sp_ofs), sp_b, new_sp_ofs.
        ...
        - (* sp_writable *)
          ...
          intros ofs' Hofs'.
          eapply Mem.perm_store_1. exact Hstore_env.
