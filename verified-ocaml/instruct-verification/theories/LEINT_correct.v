...
Proof.
  ...
  set (le_bool := Int64.cmp Cle (Int64.repr (a * 2 + 1)) (Int64.repr (b * 2 + 1))).
  set (result_v := Vlong (Int64.add (Int64.repr (if le_bool then 2 else 0)) (Int64.repr 1))).
  ...
  { apply (eval_stmt_to_exec clight_ge 15). eval_cbn.
    ...
    rewrite sem_cast_long_vlong; eval_cbn.
    rewrite sem_le_long_long; eval_cbn.
    rewrite sem_cast_bool_int_to_long; eval_cbn.
    rewrite (sem_shl_bool_long_1 le_bool); eval_cbn.
...
Proof.
  ...
  - intros e le m s ard _ Hsio.
    ...
    unfold le_int_range_pre. rewrite Ha, Hs. exact (conj Hra Hrb).
Qed.
