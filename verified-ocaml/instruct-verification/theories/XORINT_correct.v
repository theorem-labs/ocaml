...
Proof.
  ...
  set (result_v := Vlong (Int64.or (Int64.xor (Int64.repr (a * 2 + 1)) (Int64.repr (b * 2 + 1))) (Int64.repr 1))).
  ...
  { apply (eval_stmt_to_exec clight_ge 10). eval_cbn.
    ...
    (* -- XORINT-specific: evaluate (Ecast (Ebinop Oor (Ebinop Oxor ...) ...)) -- *)
    ...
    rewrite sem_cast_long_vlong; eval_cbn.
    rewrite sem_xor_long_long; eval_cbn.
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    { exists result_v. split. exact Haccu_load'. simpl. unfold result_v. rewrite tagged_xorint_arith. constructor. }
    { exists new_sp_v, sp_b, (Ptrofs.add sp_ofs (Ptrofs.repr 8)).
      ...
      split; [| split; [| split; [| split; [| split; [| split; [| split; [| split]]]]]]]. exact Hsp_load'. reflexivity. simpl.
