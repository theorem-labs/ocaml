(* RERAISE_correct.v -- RERAISE handler correctness proof.
   ...
   RERAISE has the same C body as RAISE (both call do_raise), but with
   different backtrace behavior at the C level.  The Rocq model treats
   RAISE, RERAISE, and RAISE_NOTRACE identically: all map to do_raise.
   ...
   KNOWN LIMITATION: Same Vptr/Vlong gap as PUSHTRAP / RAISE.
...
Lemma skipn_cons_eq_reraise : forall {A : Type} (k : nat) (l : list A) (x : A) (xs : list A),
  skipn k l = x :: xs ->
  skipn (S k) l = xs.
Proof.
  intros A k. revert k.
              ...
              ** destruct frame3 as [| v3 rest].
                 ...
                 --- destruct v3 as [saved_ea | | |].
                     +++ (* Val_int saved_ea -- Step case *)
                         ...
                         unfold reraise_step_pre in Hstep_pre.
