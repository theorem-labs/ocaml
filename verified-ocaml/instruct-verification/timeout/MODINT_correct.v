...
Proof.
  ...
  (* ================================================================ *)
  {
    ...
    destruct (store_succeeds_from_load m1 sb (Ptrofs.unsigned so + 8)
                (Vlong a_tagged) result_v Haccu_load_m1)
      as [m' Hstore2].
    ...
    (* ============================================================== *)
    {
      ...
      (* === Sassign (s->sp = _t'1 + 1) : store new sp === *)      rewrite PTree.gso by (simpl; congruence).      rewrite Hle_s; eval_cbn.      rewrite PTree.gss; eval_cbn.      rewrite sem_add_sp_1; eval_cbn.      rewrite sem_cast_ptr_to_ptr; eval_cbn.      rewrite Mptr_Mint64; eval_cbn.      rewrite (ptrofs_add_unsigned so 16 ltac:(lia) ltac:(lia)).      unfold new_sp_v in Hstore1.      rewrite Hstore1; eval_cbn.
      (* === Sset _t'3 (deref _t'1) -- load stack[0] = b_tagged === *)      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gss; eval_cbn.      rewrite Hload_sp0_m1; eval_cbn.
      (* === Sset _divisor (cast(_t'3) >> 1) === *)      rewrite PTree.gss; eval_cbn.      rewrite sem_cast_long_vlong; eval_cbn.      rewrite sem_shr_long_int_1; eval_cbn.
      (* === Sifthenelse (_divisor == 0) === *)      rewrite PTree.gss; eval_cbn.      rewrite (sem_cmp_eq_long_int_0_false divisor_val m1 Hdivisor_ne_zero); eval_cbn.      rewrite bool_val_false; eval_cbn.      (* Takes the Sskip branch *)
      (* === Sset _t'2 (s->accu) -- reads from m1 === *)      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite Hle_s; eval_cbn.      rewrite Haccu_offset; eval_cbn.      rewrite (ptrofs_add_unsigned so 8 ltac:(lia) ltac:(lia)).      rewrite Haccu_load_m1; eval_cbn.
      (* === Sassign (s->accu = ...) -- compute and store result === *)      (* lvalue: s->accu *)      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite Hle_s; eval_cbn.
      (* rhs: evaluate the expression *)      (* _t'2 (accu value) *)      rewrite PTree.gss; eval_cbn.      rewrite sem_cast_long_vlong; eval_cbn.
      (* shr(_t'2, 1) *)      rewrite sem_shr_long_int_1; eval_cbn.
      (* _divisor *)      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gso by (simpl; congruence).      rewrite PTree.gss; eval_cbn.
      (* mod *)      rewrite (sem_mod_long_long a_untagged divisor_val m1                 Hdivisor_eq_false Hno_overflow); eval_cbn.
      (* cast mod result *)      rewrite sem_cast_long_vlong; eval_cbn.
      (* shl *)      rewrite sem_shl_long_int_1; eval_cbn.
      (* add 1 *)      rewrite sem_add_long_int_1; eval_cbn.
      (* final cast *)      rewrite sem_cast_long_vlong; eval_cbn.
      (* Store to accu field *)      rewrite (ptrofs_add_unsigned so 8 ltac:(lia) ltac:(lia)).      unfold result_v in Hstore2.      rewrite Hstore2; eval_cbn.
    ...
    (* ============================================================== *)    (* Part 2: abs_rel for post-state                                  *)    (* ============================================================== *)    {      exists ard.      set (uso := Ptrofs.unsigned so) in *.
      ...
      (* 3. accu field -- updated to Val_int (Z.rem a b) *)      { exists result_v. split.        - exact Haccu_load'.        - simpl.          unfold result_v, mod_result, a_untagged, divisor_val, a_tagged, b_tagged.          rewrite (tagged_modint_arith a b Hb_ne Ha_range Hb_range).          constructor. }
