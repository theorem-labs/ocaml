...
Lemma BGEINT_P_halt_False : forall z1 z2,
  forall v, P_halt_of (BGEINT z1 z2) v -> False.
Proof. intros z1 z2 v [_ H]. exact H. Qed.

Lemma BGEINT_P_ccall_False : forall z1 z2,
  forall n0 args0 s0, P_ccall_of (BGEINT z1 z2) n0 args0 s0 -> False.
Proof. intros z1 z2 n0 args0 s0 [_ H]. exact H. Qed.

Lemma BGEINT_step_step_eq :
  forall z1 z2,
...
Proof.
  ...
  all: try (exfalso; exact (BGEINT_P_halt_False _ _ _ Hs1)).
  all: try (exfalso; exact (BGEINT_P_halt_False _ _ _ Hs2)).
  all: try (exfalso; exact (BGEINT_P_ccall_False _ _ _ _ _ Hs1)).
  all: try (exfalso; exact (BGEINT_P_ccall_False _ _ _ _ _ Hs2)).
  - (* Step/Step *)
    assert (s0 = s1) by (eapply BGEINT_step_step_eq; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2]). subst. constructor.
  - (* Step/Error *)
    exfalso. eapply BGEINT_step_error_excl; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2].
  - (* Error/Step *)
    exfalso. eapply BGEINT_step_error_excl; [exact Hcorr2 | exact Hcorr1 | exact E2 | exact E1].
