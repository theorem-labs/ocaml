...
Lemma MAKEBLOCK1_P_halt_False : forall n,
  forall v, P_halt_of (MAKEBLOCK1 n) v -> False.
Proof. intros n v [_ H]. exact H. Qed.

Lemma MAKEBLOCK1_P_ccall_False : forall n,
  forall n0 args0 s0, P_ccall_of (MAKEBLOCK1 n) n0 args0 s0 -> False.
Proof. intros n n0 args0 s0 [_ H]. exact H. Qed.

Lemma MAKEBLOCK1_step_step_eq :
  forall n,
...
Lemma MAKEBLOCK1_step_error_excl :
  forall n,
...
Proof.
  ...
  all: try (exfalso; exact (MAKEBLOCK1_P_halt_False _ _ Hs1)).
  all: try (exfalso; exact (MAKEBLOCK1_P_halt_False _ _ Hs2)).
  all: try (exfalso; exact (MAKEBLOCK1_P_ccall_False _ _ _ _ Hs1)).
  all: try (exfalso; exact (MAKEBLOCK1_P_ccall_False _ _ _ _ Hs2)).
  - (* Step/Step *)
    assert (s0 = s1) by (eapply MAKEBLOCK1_step_step_eq; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2]). subst. constructor.
  - (* Step/Error *)
    exfalso. eapply MAKEBLOCK1_step_error_excl; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2].
  - (* Error/Step *)
    exfalso. eapply MAKEBLOCK1_step_error_excl; [exact Hcorr2 | exact Hcorr1 | exact E2 | exact E1].
