...
Lemma RAISE_NOTRACE_P_halt_False : forall v, P_halt_of RAISE_NOTRACE v -> False.
Proof. intros v [_ H]. exact H. Qed.

Lemma RAISE_NOTRACE_P_ccall_False : forall n0 args0 s0, P_ccall_of RAISE_NOTRACE n0 args0 s0 -> False.
Proof. intros n0 args0 s0 [_ H]. exact H. Qed.

Lemma RAISE_NOTRACE_step_step_eq :
...
Lemma RAISE_NOTRACE_step_error_excl :
...
Proof.
  ...
  all: try (exfalso; exact (RAISE_NOTRACE_P_halt_False _ Hs1)).
  all: try (exfalso; exact (RAISE_NOTRACE_P_halt_False _ Hs2)).
  all: try (exfalso; exact (RAISE_NOTRACE_P_ccall_False _ _ _ Hs1)).
  all: try (exfalso; exact (RAISE_NOTRACE_P_ccall_False _ _ _ Hs2)).
  - (* Step/Step *)
    assert (s0 = s1) by (eapply RAISE_NOTRACE_step_step_eq; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2]). subst. constructor.
  - (* Step/Error *)
    exfalso. eapply RAISE_NOTRACE_step_error_excl; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2].
  - (* Error/Step *)
    exfalso. eapply RAISE_NOTRACE_step_error_excl; [exact Hcorr2 | exact Hcorr1 | exact E2 | exact E1].
