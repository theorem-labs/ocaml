...
Lemma OFFSETREF_P_halt_False : forall z,
  forall v, P_halt_of (OFFSETREF z) v -> False.
Proof. intros z v [_ H]. exact H. Qed.

Lemma OFFSETREF_P_ccall_False : forall z,
  forall n0 args0 s0, P_ccall_of (OFFSETREF z) n0 args0 s0 -> False.
Proof. intros z n0 args0 s0 [_ H]. exact H. Qed.

Lemma OFFSETREF_step_step_eq :
  forall z,
...
Lemma OFFSETREF_step_error_excl :
  forall z,
...
Proof.
  ...
  all: try (exfalso; exact (OFFSETREF_P_halt_False _ _ Hs1)).
  all: try (exfalso; exact (OFFSETREF_P_halt_False _ _ Hs2)).
  all: try (exfalso; exact (OFFSETREF_P_ccall_False _ _ _ _ Hs1)).
  all: try (exfalso; exact (OFFSETREF_P_ccall_False _ _ _ _ Hs2)).
  - (* Step/Step *)
    assert (s0 = s1) by (eapply OFFSETREF_step_step_eq; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2]). subst. constructor.
  - (* Step/Error *)
    exfalso. eapply OFFSETREF_step_error_excl; [exact Hcorr1 | exact Hcorr2 | exact E1 | exact E2].
  - (* Error/Step *)
    exfalso. eapply OFFSETREF_step_error_excl; [exact Hcorr2 | exact Hcorr1 | exact E2 | exact E1].
