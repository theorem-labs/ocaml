...
Theorem verify_BNEQ_correct : forall n target,
    Int.min_signed <= n <= Int.max_signed ->
...
Proof.
  ...
  (* ================================================================ *)
  {
    destruct (Z.eqb a n) eqn:Heqb.

    (* ============================================================== *)
    (* Case 1a: a = n => fall through (else branch in C: One is false) *)
    (* ============================================================== *)
    { apply Z.eqb_eq in Heqb. subst a. simpl.
      ...
      destruct interp_state_co_bneq as [co_is [Hco [Hpc_offset Haccu_offset]]].
      ...
      assert (Haccu_m1 : Mem.load Mint64 m1 sb (Ptrofs.unsigned so + 8) = Some (Vlong (Int64.repr (n * 2 + 1)))).
      ...
      set (le' := PTree.set _t'4 (Vptr cb pc1) (PTree.set _t'3 (Vlong (Int64.repr (n * 2 + 1)))
      ...
      (* Part 1: manual bigstep *)
      {
        ...
        set (le3 := PTree.set _t'3 (Vlong (Int64.repr (n * 2 + 1))) le2).
        ...
        { apply exec_Sseq_1 with (t1 := E0) (le1 := le1) (m1 := m1).
          ...
          { apply exec_Sseq_1 with (t1 := E0) (le1 := le2) (m1 := m1).
            ...
            { apply exec_Sseq_1 with (t1 := E0) (le1 := le3) (m1 := m1).
              ...
              { (* Sifthenelse: One comparison false => else branch (fall through) *)
                ...
                { eapply eval_Ebinop.
                  ...
                  - eapply eval_Ecast.
                    ...
                    + apply sem_cast_long_vlong.
                  - apply (bneq_cmp_eq n m1 Haccu_range Hn_range'). }
      ...
      (* Part 2: abs_rel *)
      {
        ...
        assert (Ha2 : Mem.load Mint64 m2 sb (uso + 8) = Some (Vlong (Int64.repr (n * 2 + 1)))).
        { assert (Hx : Mem.load Mint64 m1 sb (uso + 8) = Some (Vlong (Int64.repr (n * 2 + 1)))) by (prove_field_survives Hs1 Haccu_load).
        ...
        - exists (Vlong (Int64.repr (n * 2 + 1))). split. exact Ha2. simpl. rewrite Haccu_eq. constructor.
    ...
    (* ============================================================== *)
    (* Case 1b: a <> n => branch taken (then branch in C: One is true) *)
    (* ============================================================== *)
    { apply Z.eqb_neq in Heqb. simpl.
      intros ard Hpre Hstep_pre.
      unfold abs_rel_with_ard in Hpre.
      set (sb := ar_sptr_block ard) in *. set (so := ar_sptr_ofs ard) in *.
      set (hm := ar_heap_map ard) in *.
        ...
        { (* Preamble *)
          ...
          { (* Sset _t'2 ; (Sset _t'3 ; Sifthenelse) *)
            ...
            { (* Sset _t'3 ; Sifthenelse *)
              ...
              { (* Sifthenelse: One comparison true => then branch (branch taken) *)
                eapply exec_Sifthenelse.
                ...
                { (* Eval condition *)
                  ...
                  - eapply eval_Ecast.
                    + eapply eval_Ebinop.
                      ...
                      * apply sem_shr_long_int_1.
                    + apply sem_cast_long_vlong.
                  - apply (bneq_cmp_neq a n m1 Haccu_range Hn_range' Heqb). }
  ...
  (* ================================================================ *)
  - destruct fields as [| h t];
      simpl; intros ard _ Hstep; destruct Hstep as (_ & _ & _ & _ & H & _); destruct H.
  - simpl; intros ard _ Hstep; destruct Hstep as (_ & _ & _ & _ & H & _); destruct H.
  - simpl; intros ard _ Hstep; destruct Hstep as (_ & _ & _ & _ & H & _); destruct H.
Qed.

(* Wrapper with building-block precondition for Module Type *)
Theorem verify_BNEQ_handler_correct : forall n target,
    Int.min_signed <= n <= Int.max_signed ->
    handler_correct (handle_BNEQ n target) f_instr_BNEQ
...
Proof.
  ...
  - exact (verify_BNEQ_correct n target Hn).
...
(* Wrapper with the canonical type expected by InstructVerificationProof.v.
   ...
   Error "BNEQ: malformed operand", which matches P_error_of because
...
Proof.
  ...
  - (* In range: extract the signed-range fact from Hwfb *)
