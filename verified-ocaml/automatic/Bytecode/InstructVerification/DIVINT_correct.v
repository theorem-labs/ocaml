...
Proof.
  ...
  - (* b <> 0: normal division *)
    ...
    destruct Hstep_pre as (Hb_ne & Harange & Hbrange & Hint_tagged_a & Hint_tagged_b).
    ...
    (* sp_b <> sb is now Hsp_ne_sb from abs_rel destruct *)
         ...
         Ssequence
           (Ssequence                    -- PartA
             ...
             (Ssequence
               ...
               (Sset _divisor (shr (cast _t'3) 1))))
           (Ssequence                    -- PartB
             (Sifthenelse (_divisor == 0) (Scall ...) Sskip)
             (Ssequence
               (Ssequence
                 (Sset _t'2 (s->accu))
                 (Sassign (s->accu) ...result...))
               (Sreturn 0)))
      *)

      (* Intermediate temp envs *)
      set (le1 := PTree.set _t'1 (Vptr sp_b sp_ofs) le).
      set (le2 := PTree.set _t'3 (Vlong b_tagged) le1).
      set (le3 := PTree.set _divisor (Vlong divisor) le2).

      (* le' = PTree.set _t'2 ... le3 — verify the definition matches *)
      ...
      {
        (* PartA = Ssequence (Sset_t'1 ; Sassign_sp) (Sset_t'3 ; Sset_divisor) *)
        apply exec_Sseq_1 with (t1 := E0) (le1 := le1) (m1 := m1).

        (* -- First half: Sset _t'1 ; Sassign s->sp -- *)
        {
          apply exec_Sseq_1 with (t1 := E0) (le1 := le1) (m1 := m).

          (* Sset _t'1 (s->sp) : read sp field *)
          {
            apply exec_Sset.
            eapply eval_Elvalue.
          ...
          (* Sassign (s->sp) (_t'1 + 1) : write sp field *)
          {
            eapply exec_Sassign.
            - eapply eval_Efield_struct.
              + eapply eval_Elvalue.
        ...
        (* -- Second half: Sset _t'3 ; Sset _divisor -- *)
        {
          apply exec_Sseq_1 with (t1 := E0) (le1 := le2) (m1 := m1).

          (* Sset _t'3 (deref _t'1) : deref old sp to get stack top *)
          {
            apply exec_Sset.
            eapply eval_Elvalue.
            - eapply eval_Ederef. eapply eval_Etempvar.
      ...
      (* ====== PartB: branch + div + store + return ====== *)
      {
        (* Sifthenelse ; rest *)
        apply exec_Sseq_1 with (t1 := E0) (le1 := le3) (m1 := m1).

        (* -- Sifthenelse (_divisor == 0) (Scall ...) Sskip -- *)
        (* Condition is false (divisor != 0), so we take the Sskip branch *)
        {
          ...
          - rewrite Hdivisor_ne. apply bool_val_of_bool_tint.
          - (* false branch: Sskip *)
        ...
        (* (Sset _t'2 (s->accu)) ; (Sassign (s->accu) ...) *)
        {
          ...
          (* -- Sassign (s->accu) (shl(div(shr(cast _t'2,1), _divisor),1)+1) -- *)
          {
            eapply exec_Sassign.
            - (* lvalue: s->accu *)
              eapply eval_Efield_struct.
              ...
              + exact Hco.
              + exact Haccu_offset.
            - (* rvalue: ((cast _t'2 >> 1) / _divisor) << 1 + 1 *)
              eapply eval_Ebinop.
              + (* shl(div(...), 1) *)
                eapply eval_Ebinop.
                * (* cast(div(shr(cast _t'2, 1), _divisor)) *)
                  eapply eval_Ecast.
                  { eapply eval_Ebinop.
                    - (* shr(cast _t'2, 1) *)
                      eapply eval_Ebinop.
                    ...
                    - unfold a_tagged, a_untagged, divisor, b_tagged, div_result in *.
                      apply sem_div_shr_succeeds. exact Hdivisor_ne. }
                  { apply sem_cast_long_vlong. }
                * eapply eval_Econst_int.
    ...
    (* ============================================================== *)    (* Part 2: abs_rel for post-state                                  *)    (* ============================================================== *)    {      exists ard.      set (uso := Ptrofs.unsigned so) in *.
      ...
      { exists pc_ptr. split.
        - exact Hpc_load'.
        - simpl. exact Hpc_rel. }
      (* 3. accu field -- updated to Val_int (Z.quot a b) *)
      split.
      { exists result_v. split.
        - exact Haccu_load'.
        - simpl.
          unfold result_v, div_result, a_untagged, divisor, a_tagged, b_tagged.
          rewrite tagged_divint_arith by assumption.
          apply vr_int. }
      ...
      (* 9. sb writable -- preserved across stores *)
      intros ofs' Hofs'.
      eapply Mem.perm_store_1. exact Hstore2.
      eapply Mem.perm_store_1. exact Hstore1.
