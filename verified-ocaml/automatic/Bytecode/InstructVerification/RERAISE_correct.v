(* RERAISE_correct.v -- RERAISE handler correctness proof.

   RERAISE pops a trap frame from the stack, restoring pc, sp, trap_sp,
   env, and extra_args.  This is the exception-re-raising instruction.

   C code (f_instr_RERAISE), live path (dead Sifthenelse branch elided):
...
Lemma interp_state_co_reraise : exists co,
...
Theorem verify_RERAISE_correct :
...
Proof.
  ...
  - set (k := Nat.sub (length (Machine.stack s)) (trap_sp s)).
    ...
    + destruct v0 as [handler_pc | | |]; try (right; reflexivity).
      ...
      destruct interp_state_co_reraise as [co_is [Hco [Hpc_offset [Hsp_offset [Henv_offset [Hextra_offset Htrap_offset]]]]]].
      ...
      (* Part 1: exec *)
      {
        ...
        { eapply exec_Sifthenelse with (b := false).
          ...
          - reflexivity.
          - constructor. }
        (* Now handle the live RERAISE code. *)
...
(* Wrapper with the uniform type expected by InstructVerificationProof.v.
   handle_instr RERAISE / clight_of RERAISE / pre_of RERAISE are
   convertible with (fun _ s => do_raise s.(accu) s) / f_instr_RERAISE /
   raise_step_pre.  The Step case is delegated to verify_RERAISE_correct.
...
Proof.
  intros e le m s.
  change (handle_instr RERAISE (Machine.pc s) s)
    with (do_raise s.(accu) s).
  pose proof (verify_RERAISE_correct e le m s) as H.
