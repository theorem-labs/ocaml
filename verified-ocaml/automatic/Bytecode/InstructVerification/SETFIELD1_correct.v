...
Proof.
  ...
  destruct (set_nth fields 1 newval) as [new_fields|] eqn:Hset.
  2: { simpl. exact Hset. }

  {
    ...
    { apply (load_after_store_other m m1 sb
               ...
               new_sp_v (Vptr hb hofs) Hstore1 Haccu_load). left. lia. }

    assert (Hload_sp0_m1 : Mem.load Mint64 m1 sp_b (Ptrofs.unsigned sp_ofs) =
              Some stk_top_cv).
    { erewrite Mem.load_store_other.
      - exact Hload_sp0. - exact Hstore1. - left. exact Hsp_ne_sb. }
    ...
    assert (Haccu_load_m2 : Mem.load Mint64 m2 sb (Ptrofs.unsigned so + 8) =
              Some (Vptr hb hofs)).
    { erewrite Mem.load_store_other.
      - exact Haccu_load_m1. - exact Hstore2.
    ...
    (* Part 1: exec *)
    {
      ...
      rewrite Hle_s; eval_cbn. rewrite Hco; eval_cbn.
      rewrite Hsp_offset; eval_cbn. rewrite Mptr_Mint64; eval_cbn.
      ...
      rewrite sem_add_sp_1; eval_cbn. rewrite sem_cast_ptr_to_ptr; eval_cbn.
      rewrite (ptrofs_add_unsigned so 16 ltac:(lia) ltac:(lia)).
      ...
      (* S5: *(cast(_t'2) + 1) = _t'3 -- field 1, add offset 8 *)
      rewrite PTree.gso by (compute; congruence). rewrite PTree.gss; eval_cbn.
    ...
    (* Part 2: abs_rel *)
    {
      ...
      { intros ofs0 v0 Hld. erewrite Mem.load_store_other; [exact Hld | exact Hstore2 |].
        left. intro Heq. exact (Hhb_ne_sb (eq_sym Heq)). }
      ...
      { pose proof (load_after_store_same m m1 sb (uso + 16) new_sp_v Hstore1) as Htmp.
        unfold new_sp_v in Htmp |- *. simpl Val.load_result in Htmp. rewrite ptr64_true in Htmp. exact Htmp. }
      ...
      { intros ofs' Hofs'. eapply Mem.perm_store_1. exact Hstore3.
        eapply Mem.perm_store_1. exact Hstore2. eapply Mem.perm_store_1. exact Hstore1.
