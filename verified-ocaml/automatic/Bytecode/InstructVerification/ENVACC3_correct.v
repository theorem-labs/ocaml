(* ENVACC3_correct.v -- ENVACC3 correctness proof.

   ENVACC3: accu = Field(env, 3), i.e., load field 3 from the closure
   environment pointed to by env.

   C handler: t1 = s->env; t2 = deref((long ptr)t1 + 3); s->accu = t2; return 0;
...
Lemma sem_cast_long_to_ptr_vptr_EA3 : forall b ofs m,
...
Definition env_field_loadable_3
  ...
  forall v,
    ...
    forall env_v,
      ...
      exists b ofs cv,
        ...
        b <> sb /\
        Mem.load Mint64 m b
          (Ptrofs.unsigned (Ptrofs.add ofs (Ptrofs.repr (Z.of_nat 3 * 8)))) = Some cv /\
...
Proof.
  ...
  2: { reflexivity. }
  {
    ...
    unfold env_field_loadable_3 in Hefl.
    ...
    (* Part 1: exec *)
    {
      ...
      rewrite sem_cast_long_to_ptr_vptr_EA3; eval_cbn.
      rewrite (sem_add_ptr_long_3 b ofs m); eval_cbn.
