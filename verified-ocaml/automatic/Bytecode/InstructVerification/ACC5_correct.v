...
Theorem verify_ACC5 :
    handler_correct_v1 (handle_ACC 5) f_instr_ACC5
      (fun _ _ _ _ => True)
...
Proof.
  ...
  destruct (store_succeeds_sb m sb so 8 accu_v Hsb_writable Haccu_load ltac:(lia) ltac:(lia) cv5) as [m' Hstore].
  ...
  { apply (eval_stmt_to_exec clight_ge 10). eval_cbn.
    ...
    rewrite (sem_cast_long_val_repr _ _ _ _ _ _ Hval_repr5); eval_cbn.
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    assert (Haccu_load' : Mem.load Mint64 m' sb (uso + 8) = Some cv5). { pose proof (load_after_store_same m m' sb (uso + 8) cv5 Hstore) as Htmp. rewrite (val_repr_load_result hm cb co v5 cv5 Hval_repr5) in Htmp. exact Htmp. }
    ...
    { exists (Vptr sp_b sp_ofs), sp_b, sp_ofs. split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split]]]]]]]]. exact Hsp_load'. reflexivity. simpl. rewrite Hstk.
            apply (stack_repr_store_other_block hm cb co m m' _ sp_b sp_ofs sb (uso + 8) cv5 Hstack_repr Hstore). intro Heq; exact (Hsp_ne_sb (eq_sym Heq)).
        - exact Hsp_ne_sb.
        - exact Hsp_ne_gb.
        - exact Hcb_ne_sp.
        - exact Hsp_ge8.
    ...
    { exists gd_ptr. split; [| split; [| split]]. exact Hgd_load'. simpl. exact Hgd_eq. simpl. apply (global_repr_store_other_block hm cb co m m' _ _ _ sb (uso + 8) cv5 Hglobal_repr Hstore). intro Heq2; exact (Hgb_ne (eq_sym Heq2)). exact Hgb_ne_sb. }
