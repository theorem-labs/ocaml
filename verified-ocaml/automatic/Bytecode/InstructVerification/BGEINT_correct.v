...
Theorem verify_BGEINT_correct : forall n target,
    Int.min_signed <= n <= Int.max_signed ->
    handler_correct_v1 (handle_BGEINT n target) f_instr_BGEINT
      (fun _ m s ard =>
...
Theorem verify_BGEINT_handler_correct_v1 : forall n target,
    Int.min_signed <= n <= Int.max_signed ->
    handler_correct_v1 (handle_BGEINT n target) f_instr_BGEINT
...
Proof.
  ...
  - exact (verify_BGEINT_correct n target Hn).
...
(* Wrapper with the canonical type expected by InstructVerificationProof.v.
   ...
   The handler checks instr_wfb (BGEINT z1 z2) = ((min_signed <=? z1) &&
   (z1 <=? max_signed))%Z.  When that guard is false the handler returns
   Error "BGEINT: malformed operand", which matches error_message_of because
...
Theorem correct_BGEINT : forall z1 z2,
  handler_correct (handle_instr (Bytecode.AST.BGEINT z1 z2)) (clight_of (Bytecode.AST.BGEINT z1 z2))
    (error_message_of (Bytecode.AST.BGEINT z1 z2))
    (pre_of (Bytecode.AST.BGEINT z1 z2)) (P_halt_of (Bytecode.AST.BGEINT z1 z2)) (P_ccall_of (Bytecode.AST.BGEINT z1 z2)).
Proof.
Admitted.
