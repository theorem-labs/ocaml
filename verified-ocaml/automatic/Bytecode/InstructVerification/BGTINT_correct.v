(* BGTINT_correct.v -- BGTINT handler completeness proof (manual bigstep).

   Rocq handler:
     handle_BGTINT n target pc' s =
       match s.(accu) with
...
Lemma interp_state_co_bgtint : exists co,
...
(* Key lemma: when n > a, the C comparison (long)n > (long)(accu>>1) is true.
   C Ogt gives Int64.lt (repr a) (repr n) which equals Z.ltb a n = Z.gtb n a = true. *)
Local Lemma bgtint_cmp_true : forall n a m,
  -4611686018427387904 <= a <= 4611686018427387903 ->
...
Proof.
  intros n0 a0 m0 Ha Hn Hgt. rewrite tagged_shr_eq by lia.
  rewrite sem_gt_long_long. rewrite Int.signed_repr by exact Hn.
  f_equal. f_equal.
  rewrite Z.gtb_ltb in Hgt. apply Z.ltb_lt in Hgt.
  unfold Int64.lt.
...
(* Key lemma: when n <= a (gtb is false), the C comparison is false. *)
Local Lemma bgtint_cmp_false : forall n a m,
  -4611686018427387904 <= a <= 4611686018427387903 ->
  Int.min_signed <= n <= Int.max_signed ->
  Z.gtb n a = false ->
  sem_binary_operation (genv_cenv clight_ge) Ogt
...
Proof.
  ...
  rewrite sem_gt_long_long. rewrite Int.signed_repr by exact Hn.
  f_equal. f_equal.
  rewrite Z.gtb_ltb in Hgt. apply Z.ltb_ge in Hgt.
  unfold Int64.lt.
  assert (Ha64 : Int64.min_signed <= a0 <= Int64.max_signed).
  { assert (Int64.min_signed = -9223372036854775808)%Z by reflexivity.
    assert (Int64.max_signed = 9223372036854775807)%Z by reflexivity.
...
Theorem verify_BGTINT_correct : forall n target,
              ...
              { (* Sifthenelse: comparison true => then branch *)
                ...
                { (* Eval condition *)
                  ...
                  - eapply eval_Ecast.
                    ...
                    + apply sem_cast_long_vlong.
                  - apply (bgtint_cmp_true n a m1 Haccu_range Hn_range Hcmp). }
                ...
                { eapply eval_Ebinop.
                  ...
                  - eapply eval_Ecast.
                    + eapply eval_Ebinop.
                      ...
                      * apply sem_shr_long_int_1.
                    + apply sem_cast_long_vlong.
                  - apply (bgtint_cmp_false n a m1 Haccu_range Hn_range Hcmp). }
                { apply bool_val_of_bool. }
                { simpl.
...
(* Wrapper with the canonical type expected by InstructVerificationProof.v.
   ...
   The existing proof (verify_BGTINT_handler_correct) requires
   ...
   handler's Z.gtb, so the Step case cannot be closed without
   ...
   case delegates to verify_BGTINT_handler_correct after asserting
   the range (Admitted). *)
Theorem correct_BGTINT : forall z1 z2,
  handler_correct (handle_instr (Bytecode.AST.BGTINT z1 z2)) (clight_of (Bytecode.AST.BGTINT z1 z2))
    (pre_of (Bytecode.AST.BGTINT z1 z2))
    (P_error_of (Bytecode.AST.BGTINT z1 z2)) (P_halt_of (Bytecode.AST.BGTINT z1 z2)) (P_ccall_of (Bytecode.AST.BGTINT z1 z2)).
Proof.
  intros z1 z2 e le m s.
  change (handle_instr (Bytecode.AST.BGTINT z1 z2)) with (handle_BGTINT z1 z2).
  unfold handle_BGTINT.
  ...
  - (* Val_int z — Step case; need z1 range for C comparison correctness *)
    ...
    { (* TODO: the canonical pre_of does not include this guard;
         ...
         Admitted for now. *)
      admit. }
    destruct (Z.gtb z1 z) eqn:Hcmp; simpl;
    (intros ard Hrel Hpre;
     pose proof (verify_BGTINT_handler_correct z1 z2 Hn) as Hvc;
     specialize (Hvc e le m s);
     unfold handler_correct, handle_BGTINT in Hvc;
