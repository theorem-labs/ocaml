...
Proof.
  ...
  (* ================================================================ *)
  {
    ...
    { simpl.
      ...
      destruct interp_state_co_bleint as [co_is [Hco [Hpc_offset Haccu_offset]]].
              ...
              { (* Sifthenelse: comparison true => then branch *)
                ...
                { (* Eval condition *)
                  ...
                  - eapply eval_Ecast.
                    ...
                    + apply sem_cast_long_vlong.
                  - apply (bleint_cmp_true n a m1 Haccu_range Hn_range Hcmp). }
                ...
                { eapply eval_Ebinop.
                  ...
                  - eapply eval_Ecast.
                    + eapply eval_Ebinop.
                      ...
                      * apply sem_shr_long_int_1.
                    + apply sem_cast_long_vlong.
                  - apply (bleint_cmp_false n a m1 Haccu_range Hn_range Hcmp). }
                { apply bool_val_of_bool. }
                { simpl.
...
Proof.
  ...
  - exact (verify_BLEINT_correct n target).
...
(* Wrapper with the canonical type expected by InstructVerificationProof.v.
   ...
   The existing proof (verify_BLEINT_handler_correct) requires
   ...
   handler's Z.leb, so the Step case cannot be closed without
   ...
   case delegates to verify_BLEINT_handler_correct after asserting
   the range (Admitted). *)
Theorem correct_BLEINT : forall z1 z2,
  handler_correct (handle_instr (Bytecode.AST.BLEINT z1 z2)) (clight_of (Bytecode.AST.BLEINT z1 z2))
    (pre_of (Bytecode.AST.BLEINT z1 z2))
    (P_error_of (Bytecode.AST.BLEINT z1 z2)) (P_halt_of (Bytecode.AST.BLEINT z1 z2)) (P_ccall_of (Bytecode.AST.BLEINT z1 z2)).
Proof.
  intros z1 z2 e le m s.
  change (handle_instr (Bytecode.AST.BLEINT z1 z2)) with (handle_BLEINT z1 z2).
  unfold handle_BLEINT.
  ...
  - (* Val_int z — Step case; need z1 range for C comparison correctness *)
    ...
    { (* TODO: the canonical pre_of does not include this guard;
         ...
         Admitted for now. *)
      admit. }
    destruct (Z.leb z1 z) eqn:Hcmp; simpl;
    (intros ard Hrel Hpre;
     pose proof (verify_BLEINT_handler_correct z1 z2 Hn) as Hvc;
     specialize (Hvc e le m s);
     unfold handler_correct, handle_BLEINT in Hvc;
