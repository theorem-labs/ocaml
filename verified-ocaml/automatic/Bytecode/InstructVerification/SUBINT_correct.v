...
Theorem verify_SUBINT_correct :
    handler_correct_v1 handle_SUBINT f_instr_SUBINT
...
Proof.
  intros e le m s. unfold handler_correct_v1, handle_SUBINT.
  ...
  set (result_v := Vlong (Int64.add (Int64.sub (Int64.repr (a * 2 + 1)) (Int64.repr (b * 2 + 1))) (Int64.repr 1))).
  ...
  { apply (eval_stmt_to_exec clight_ge 10). eval_cbn.
    ...
    rewrite sem_cast_long_vlong; eval_cbn.
    rewrite sem_sub_long_long; eval_cbn.
  ...
  { exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    { exists result_v. split. exact Haccu_load'. simpl. unfold result_v. rewrite tagged_subint_arith. apply vr_int. }
    { exists new_sp_v, sp_b, (Ptrofs.add sp_ofs (Ptrofs.repr 8)). split; [| split; [| split; [| split; [| split; [| split; [| split; [| split; [| split]]]]]]]]. exact Hsp_load'. reflexivity. simpl.
