...
Proof.
  ...
  all: try (intros ard0 _ Hpre_false; unfold eq_int_range_pre in Hpre_false;
  ...
  (* Extract range and Vlong representation constraints from precondition *)
  unfold eq_int_range_pre in Hrange. rewrite Haccu_eq, Hstk in Hrange.
  ...
  2: { (* vr_code_ptr case: Val_int a represented as Vptr, but int_vlong says Vlong *)
       exfalso.
       destruct (Hint_tagged_a _ Haccu_repr) as [z Hz].
       discriminate Hz. }
  ...
  2: { (* vr_code_ptr case: Val_int b represented as Vptr, but int_vlong says Vlong *)
       exfalso.
       destruct (Hint_tagged_b _ Hval_repr0) as [z Hz].
       discriminate Hz. }
  ...
  set (eq_bool := Int64.eq (Int64.repr (a * 2 + 1)) (Int64.repr (b * 2 + 1))).
  set (result_v := Vlong (Int64.add (Int64.repr (if eq_bool then 2 else 0)) (Int64.repr 1))).
  ...
  { apply (eval_stmt_to_exec clight_ge 15). eval_cbn.
    ...
    (* === Sassign (s->accu = ((long)((long)_t'2 == (long)_t'3) << 1) + 1) === *)
    (* lvalue: s->accu -- skip _t'3, _t'2, _t'1 to read _s *)
    rewrite PTree.gso by (compute; congruence). rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence). rewrite Hle_s; eval_cbn.
    (* rvalue: _t'2 lookup -- skip _t'3 to get _t'2 *)
    rewrite PTree.gso by (compute; congruence). rewrite PTree.gss; eval_cbn.
    (* cast _t'2 to long *)
    ...
    (* cast _t'3 to long *)
    rewrite sem_cast_long_vlong; eval_cbn.
    (* Oeq: (long)_t'2 == (long)_t'3 *)
    rewrite sem_eq_long_long; eval_cbn.
    (* Ecast bool->long: (long)(... == ...) *)
    rewrite sem_cast_bool_int_to_long; eval_cbn.
    (* Oshl: (long)(...) << 1 *)
    rewrite sem_shl_bool_long_1; eval_cbn.
    (* Oadd: (...) + 1 *)
    rewrite sem_add_long_int_1; eval_cbn.
    (* final cast for assignment *)
    rewrite sem_cast_long_vlong; eval_cbn.
    (* store to accu field *)
    rewrite (ptrofs_add_unsigned so 8 ltac:(lia) ltac:(lia)).
    fold eq_bool. fold result_v. rewrite Hstore2; eval_cbn.
  ...
  { unfold abs_rel. exists ard. set (uso := Ptrofs.unsigned so) in *.
    ...
    { exists result_v. split. exact Haccu_load'. simpl.
      unfold result_v. rewrite tagged_eq_result.
      unfold eq_bool. rewrite (tagged_eq_arith a b Ha_range Hb_range).
      destruct (Z.eqb a b); apply vr_int. }
