...
Proof.
  ...
  - set (k := Nat.sub (length (Machine.stack s)) (trap_sp s)).
    ...
    + destruct v0 as [handler_pc | | |]; try (right; reflexivity).
      ...
      destruct interp_state_co_raise as [co_is [Hco [Hpc_offset [Hsp_offset [Henv_offset [Hextra_offset Htrap_offset]]]]]].
      ...
      (* Part 1: exec *)
      {
        ...
        { eapply exec_Sifthenelse with (b := false).
          ...
          - reflexivity.
          - constructor. }
        (* Now handle the live RAISE code. *)
...
Proof.
  intros e le m s.
  change (handle_instr RAISE (Machine.pc s) s)
    with (do_raise s.(accu) s).
  pose proof (verify_RAISE_correct e le m s) as H.
