...
Proof.
  ...
  (* Normalize nested Ptrofs.add: sp + 8*5 = sp + 40 *)
  ...
  (* After stores 1 and 2, load v5's repr from the stack.
     The load at sp+40 (= new_sp+48) survives both stores. *)
  assert (Hload_sp5_m1 : Mem.load Mint64 m1 sp_b (Ptrofs.unsigned (Ptrofs.add sp_ofs (Ptrofs.repr 40))) = Some cv5).
  { erewrite Mem.load_store_other. exact Hload_sp5. exact Hstore_sp.
  ...
  { subst new_sp_ofs. rewrite Ptrofs.sub_add_opp.
    ...
    cut (Ptrofs.unsigned (Ptrofs.add (Ptrofs.neg (Ptrofs.repr 8)) (Ptrofs.repr 48))
         = Ptrofs.unsigned (Ptrofs.repr 40)).
    { intro H. rewrite <- (Ptrofs.repr_unsigned (Ptrofs.add _ _)).
      rewrite H. apply Ptrofs.repr_unsigned. }
    native_compute. reflexivity. }

  (* Store 3: accu field <- cv5 *)
  ...
  destruct (store_succeeds_sb m2 sb so 8 accu_v Hsb_writable_m2 Haccu_load_m2 ltac:(lia) ltac:(lia) cv5)
    as [m3 Hstore_accu_field].

  (* Define le' with all the temps. *)
  set (le' := PTree.set _t'3 cv5
  ...
  (* ============================================================== *)
  {
    ...
    (* Statement 7: Sset _t'3 (deref (_t'2 + 6)) -- load *(new_sp + 6) = stack[5] *)
    rewrite PTree.gss; eval_cbn.
    rewrite sem_add_sp_6; eval_cbn.
    rewrite Hadd_back.
    rewrite Hload_sp5_m2; eval_cbn.

    (* Statement 8: Sassign (s->accu) _t'3 -- store v5 to accu field *)
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    ...
    try rewrite (sem_cast_long_val_repr _ _ _ _ _ _ Hval_repr5); eval_cbn.
  ...
  (* ============================================================== *)
  {
    ...
    assert (Haccu_load3 : Mem.load Mint64 m3 sb (uso + 8) = Some cv5).
    { pose proof (load_after_store_same m2 m3 sb (uso + 8) cv5 Hstore_accu_field) as Htmp.
      rewrite (val_repr_load_result hm cb co v5 cv5 Hval_repr5) in Htmp.
      exact Htmp. }
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 16)
               cv5 (Vptr sp_b new_sp_ofs) Hstore_accu_field).
    ...
    { exists (Vptr sp_b new_sp_ofs), sp_b, new_sp_ofs.
      ...
      assert (Hstack_m3 : stack_repr hm cb co m3 (Machine.accu s :: v0 :: v1 :: v2 :: v3 :: v4 :: v5 :: rest) sp_b new_sp_ofs).
