(* PUSHCONST3_correct.v -- PUSHCONST3 completeness proof.

   PUSHCONST3 = PUSH then CONST3.
   C code:
     sp = sp - 1;          // decrement sp
     *sp = accu;            // push old accu onto stack
     accu = Val_int(3);     // set accu to tagged 3 = (3<<1)+1 = 7
   Rocq:
     handle_PUSHCONSTINT 3 pc' s =
       Step (s <|pc := pc'|> <|accu := Val_int 3|> <|stack := accu :: stack|>)

   Three stores:
     Store 1: sp field (sb, uso+16) <- Vptr sp_b new_sp_ofs
     Store 2: *new_sp (sp_b, new_sp_ofs) <- accu_v
     Store 3: accu field (sb, uso+8) <- Vlong (Int64.repr 7)

   Follows the patterns of PUSH_correct.v and CONST3_correct.v. *)
...
(* Arithmetic lemmas for constant 3: (3 << 1) + 1 = 7 *)
Local Lemma sem_cast_int_to_long_3 : forall m,
  sem_cast (Vint (Int.repr 3)) tint tlong m = Some (Vlong (Int64.repr 3)).
Proof. intros. reflexivity. Qed.

Local Lemma sem_shl_long_3_1 : forall m,
  sem_binary_operation (genv_cenv clight_ge) Oshl
    (Vlong (Int64.repr 3)) tlong (Vint (Int.repr 1)) tint m
    = Some (Vlong (Int64.repr 6)).
Proof. intros. reflexivity. Qed.

Local Lemma sem_add_long_int_6_1 : forall m,
  sem_binary_operation (genv_cenv clight_ge) Oadd
    (Vlong (Int64.repr 6)) tlong (Vint (Int.repr 1)) tint m
    = Some (Vlong (Int64.repr 7)).
Proof. intros. reflexivity. Qed.

Local Lemma val_int_3_load_result :
  Val.load_result Mint64 (Vlong (Int64.repr 7)) = Vlong (Int64.repr 7).
...
Proof.
  ...
  (* Store 3: accu field <- Vlong (Int64.repr 7) *)
  ...
  destruct (store_succeeds_sb m2 sb so 8 accu_v Hsb_writable_m2 Haccu_load_m2 ltac:(lia) ltac:(lia) (Vlong (Int64.repr 7))) as [m3 Hstore_const].
  ...
  (* ============================================================== *)
  {
    ...
    (* Statement 6: Sassign (s->accu) ((3 << 1) + 1) *)
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    rewrite PTree.gso by (compute; congruence).
    rewrite Hle_s; eval_cbn.

    (* rvalue: ((cast 3 tlong) << 1) + 1 *)
    rewrite (sem_cast_int_to_long_3 m2); eval_cbn.
    rewrite (sem_shl_long_3_1 m2); eval_cbn.
    rewrite (sem_add_long_int_6_1 m2); eval_cbn.
    rewrite (sem_cast_long_vlong (Int64.repr 7)); eval_cbn.
  ...
  (* ============================================================== *)
  {
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 0)
               (Vlong (Int64.repr 7)) pc_ptr Hstore_const).
      - apply Hload_sb_m2.
    ...
    assert (Haccu_load3 : Mem.load Mint64 m3 sb (uso + 8) = Some (Vlong (Int64.repr 7))).
    { pose proof (load_after_store_same m2 m3 sb (uso + 8)
                    (Vlong (Int64.repr 7)) Hstore_const) as Htmp.
      rewrite val_int_3_load_result in Htmp.
      exact Htmp. }
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 16)
               (Vlong (Int64.repr 7)) (Vptr sp_b new_sp_ofs) Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 24)
               (Vlong (Int64.repr 7)) env_v Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 32)
               (Vlong (Int64.repr 7)) _ Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 40)
               (Vlong (Int64.repr 7)) gd_ptr Hstore_const).
      - apply Hload_sb_m2.
    ...
    { apply (load_after_store_other m2 m3 sb (uso + 8) (uso + 48)
               (Vlong (Int64.repr 7)) ts_ptr Hstore_const).
      - apply Hload_sb_m2.
    ...
    (* 3. accu field -- updated to Val_int 3 *)
    { exists (Vlong (Int64.repr 7)). split.
      - exact Haccu_load3.
      - simpl. exact (vr_int _ _ _ 3). }
