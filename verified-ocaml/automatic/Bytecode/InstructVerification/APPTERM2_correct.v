...
(* ================================================================== *)
(* Wrapper: bridge verify_APPTERM2_correct to the InstructSpec form    *)
(* ================================================================== *)

Import Bytecode.AST.

Theorem correct_APPTERM2 : forall n,
...
Proof.
  ...
  unfold handler_correct.
  intros e le m s.
  change (handle_instr (APPTERM2 n) (Machine.pc s) s)
    with (handle_APPTERM2 n s).
  unfold handle_APPTERM2.
  ...
  - destruct rest as [| arg2 rest2] eqn:Hrest.
    + (* Error case: stack = [arg1] *)
      unfold P_error_of, error_message_of. rewrite Hstk. reflexivity.
    + destruct (get_code_ptr_s s s.(Machine.accu)) as [target_pc|] eqn:Hgcp.
      * (* Step case: delegate to verify_APPTERM2_correct *)
        intros ard Habs Hpre.
        pose proof (verify_APPTERM2_correct n) as Hverify.
        unfold handler_correct in Hverify.
        specialize (Hverify e le m s).
        unfold handle_APPTERM2 in Hverify.
        rewrite Hstk in Hverify. rewrite Hgcp in Hverify.
        apply (Hverify ard); [exact Habs |].
        (* Bridge: pre_of (APPTERM2 n) uses apply3_closure_pre;
           verify uses the local appterm2_step_pre.
           These are definitionally identical after unfolding. *)
        change (pre_of (APPTERM2 n)) with (InstructSpec.appterm2_step_pre n) in Hpre.
        unfold InstructSpec.appterm2_step_pre in Hpre.
        unfold apply3_closure_pre in Hpre.
        destruct Hpre as (Hcode & Hbound & Hfits & Hge8 & Hge2 & Hlen & Hclosure & Hextra).
        refine (conj Hcode (conj Hbound (conj Hfits (conj Hge8 (conj Hge2 (conj _ (conj _ Hextra))))))).
        { rewrite Hstk in Hlen. exact Hlen. }
        { intros sp_b sp_ofs Hsp_load.
          specialize (Hclosure sp_b sp_ofs Hsp_load).
          unfold appterm2_step_pre.
          exact Hclosure. }
