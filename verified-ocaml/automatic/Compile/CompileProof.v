...
Proof.
  intros A prefix. induction prefix as [|a pfx IH]; intros rest i Hlen.
...
Proof.
  intros A prefix. induction prefix as [|a pfx IH]; intros rest i k Hlen.
  - simpl in Hlen. subst. reflexivity.
  - simpl in Hlen. subst. simpl. apply IH. reflexivity.
Qed.
...
Proof.
  unfold expr_correct. intros n fuel ce base s sv out out' prefix Heval Hpc Hout Hpfx.
  destruct fuel as [|fuel'].
  - simpl in Heval. discriminate.
  - simpl in Heval. injection Heval as Hsv Hout'.
    exists 1%nat, (Val_int n).
    assert (Hnth : nth_error (prefix ++ [CONSTINT n; STOP]) (Z.to_nat (pc s)) =
                   Some (CONSTINT n)).
    { rewrite Hpc, Nat2Z.id.
      rewrite (nth_error_prefix prefix [CONSTINT n; STOP] base Hpfx).
      reflexivity. }
    pose proof (step_constint _ _ _ Hnth) as Hstep.
    simpl. split.
    + simpl. rewrite Hstep. simpl.
      f_equal. unfold st. rewrite Hpc.
      f_equal.
      rewrite Z.add_1_r. rewrite <- Nat2Z.inj_succ. f_equal. lia.
    + subst sv. simpl. reflexivity.
Qed.

Lemma expr_correct_bool : forall b, expr_correct (Exp_bool b).
Proof.
  unfold expr_correct. intros b fuel ce base s sv out out' prefix Heval Hpc Hout Hpfx.
  destruct fuel as [|fuel'].
  - simpl in Heval. discriminate.
  - destruct b.
    + (* true *)
      simpl in Heval. injection Heval as Hsv Hout'.
      exists 1%nat, (Val_int 1).
      assert (Hnth : nth_error (prefix ++ [CONSTINT 1; STOP]) (Z.to_nat (pc s)) =
                     Some (CONSTINT 1)).
      { rewrite Hpc, Nat2Z.id.
        rewrite (nth_error_prefix prefix [CONSTINT 1; STOP] base Hpfx).
        reflexivity. }
      pose proof (step_constint _ _ _ Hnth) as Hstep.
      simpl. split.
      * simpl. rewrite Hstep. simpl.
        f_equal. unfold st. rewrite Hpc.
        f_equal. rewrite Z.add_1_r. rewrite <- Nat2Z.inj_succ. f_equal. lia.
      * subst sv. simpl. exact I.
    + (* false *)
      simpl in Heval. injection Heval as Hsv Hout'.
      exists 1%nat, (Val_int 0).
      assert (Hnth : nth_error (prefix ++ [CONSTINT 0; STOP]) (Z.to_nat (pc s)) =
                     Some (CONSTINT 0)).
      { rewrite Hpc, Nat2Z.id.
        rewrite (nth_error_prefix prefix [CONSTINT 0; STOP] base Hpfx).
        reflexivity. }
      pose proof (step_constint _ _ _ Hnth) as Hstep.
      simpl. split.
      * simpl. rewrite Hstep. simpl.
...
Proof.
  unfold expr_correct. intros fuel ce base s sv out out' prefix Heval Hpc Hout Hpfx.
  destruct fuel as [|fuel'].
  - simpl in Heval. discriminate.
  - simpl in Heval. injection Heval as Hsv Hout'.
    exists 1%nat, (Val_int 0).
...
Lemma eval_program_fuel_monotone : forall fuel fuel' prog senv out sv out',
  ...
  eval_program fuel' prog senv out = Eval_ok sv out'.
Proof.
  intros fuel fuel' prog. revert fuel fuel'.
  induction prog as [|d rest IHprog]; intros fuel fuel' senv out sv out' Heval Hle.
  - (* empty program *)
...
Proof.
  (* This proof is the main deliverable of the formal verification effort.
     ...
     PROVED (19 fully-checked lemmas, 0 axioms used):
     ...
     1. [DONE] Prove eval_fuel_monotone by strong induction on fuel.
