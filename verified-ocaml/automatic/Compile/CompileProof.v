...
Lemma let_int_var_stop_behavior_equiv : forall src_result bc_fuel n,
  Int.min_signed <= n <= Int.max_signed ->
  ((exists vsrc, src_result = Term_normal vsrc) \/ src_result = Term_timeout) ->
  behavior_equiv (mk_behavior [] src_result)
                 (bytecode_behavior bc_fuel [CONSTINT n; PUSH; ACC 0; POP 1; STOP] []).
Proof.
  intros src_result bc_fuel n Hn Hsrc.
  set (s0 := initial_state []).
  set (s1 := st s0 1 (Val_int n) [] val_unit 0 [] 0).
  set (s2 := st s1 2 (Val_int n) [Val_int n] val_unit 0 [] 0).
  ...
  destruct bc_fuel as [|[|[|[|[|fuel]]]]].
  - destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 0 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s0 s1 [] Hconst).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 1 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s0 s1 [] Hconst).
    rewrite (rc_step 0 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s1 s2 [] Hpush).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 2 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s0 s1 [] Hconst).
    rewrite (rc_step 1 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s1 s2 [] Hpush).
    rewrite (rc_step 0 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s2 s3 [] Hacc).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 3 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s0 s1 [] Hconst).
    rewrite (rc_step 2 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s1 s2 [] Hpush).
    rewrite (rc_step 1 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s2 s3 [] Hacc).
    rewrite (rc_step 0 [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s3 s4 [] Hpop).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step (S (S (S (S fuel)))) [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s0 s1 [] Hconst).
    rewrite (rc_step (S (S (S fuel))) [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s1 s2 [] Hpush).
    rewrite (rc_step (S (S fuel)) [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s2 s3 [] Hacc).
    rewrite (rc_step (S fuel) [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s3 s4 [] Hpop).
    rewrite (rc_halt fuel [CONSTINT n; PUSH; ACC 0; POP 1; STOP] s4 (Val_int n) [] Hhalt).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
Qed.

Lemma let_then_add_stop_behavior_equiv : forall src_result bc_fuel a b,
  Int.min_signed <= a <= Int.max_signed ->
  Int.min_signed <= b <= Int.max_signed ->
  ((exists vsrc, src_result = Term_normal vsrc) \/ src_result = Term_timeout) ->
  behavior_equiv (mk_behavior [] src_result)
                 (bytecode_behavior bc_fuel [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] []).
Proof.
  intros src_result bc_fuel a b Ha Hb Hsrc.
  set (s0 := initial_state []).
  set (s1 := st s0 1 (Val_int a) [] val_unit 0 [] 0).
  set (s2 := st s1 2 (Val_int a) [Val_int a] val_unit 0 [] 0).
  set (s3 := st s2 3 (Val_int b) [Val_int a] val_unit 0 [] 0).
  set (s4 := st s3 4 (Val_int b) [Val_int b; Val_int a] val_unit 0 [] 0).
  ...
  destruct bc_fuel as [|[|[|[|[|[|[|fuel]]]]]]].
  - destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s2 s3 [] Hconst_b).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s3 s4 [] Hpush_b).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 4 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s4 s5 [] Hacc).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 5 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 4 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s4 s5 [] Hacc).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s5 s6 [] Hadd).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step (S (S (S (S (S (S fuel)))))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step (S (S (S (S (S fuel))))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step (S (S (S (S fuel)))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step (S (S (S fuel))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step (S (S fuel)) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s4 s5 [] Hacc).
    rewrite (rc_step (S fuel) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s5 s6 [] Hadd).
    rewrite (rc_halt fuel [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s6 (Val_int (a + b)) [] Hhalt).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
Qed.

Lemma let_then_add_second_oob_behavior_equiv : forall src_result bc_fuel a b,
  Int.min_signed <= a <= Int.max_signed ->
  ~ (Int.min_signed <= b <= Int.max_signed) ->
  (src_result = Term_error constint_malformed_msg \/ src_result = Term_timeout) ->
  behavior_equiv (mk_behavior [] src_result)
...
Proof.
  ...
  - rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s1 s2 [] Hpush_a).
    destruct Hsrc as [-> | ->]; simpl; split; auto.
  - rewrite (rc_step (S (S fuel)) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP] s0 s1 [] Hconst_a).
...
Lemma let_add_stop_behavior_equiv : forall src_result bc_fuel a b,
  Int.min_signed <= a <= Int.max_signed ->
  Int.min_signed <= b <= Int.max_signed ->
  ((exists vsrc, src_result = Term_normal vsrc) \/ src_result = Term_timeout) ->
  behavior_equiv (mk_behavior [] src_result)
                 (bytecode_behavior bc_fuel [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] []).
Proof.
  intros src_result bc_fuel a b Ha Hb Hsrc.
  set (s0 := initial_state []).
  set (s1 := st s0 1 (Val_int a) [] val_unit 0 [] 0).
  ...
  destruct bc_fuel as [|[|[|[|[|[|[|[|fuel]]]]]]]].
  - destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 s3 [] Hconst_b).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s3 s4 [] Hpush_b).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 4 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s4 s5 [] Hacc).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 5 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 4 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s4 s5 [] Hacc).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s5 s6 [] Hadd).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step 6 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 5 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step 4 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step 3 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step 2 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s4 s5 [] Hacc).
    rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s5 s6 [] Hadd).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s6 s7 [] Hpop).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
  - rewrite (rc_step (S (S (S (S (S (S (S fuel))))))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step (S (S (S (S (S (S fuel)))))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_step (S (S (S (S (S fuel))))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 s3 [] Hconst_b).
    rewrite (rc_step (S (S (S (S fuel)))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s3 s4 [] Hpush_b).
    rewrite (rc_step (S (S (S fuel))) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s4 s5 [] Hacc).
    rewrite (rc_step (S (S fuel)) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s5 s6 [] Hadd).
    rewrite (rc_step (S fuel) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s6 s7 [] Hpop).
    rewrite (rc_halt fuel [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s7 (Val_int (a + b)) [] Hhalt).
    destruct Hsrc as [[vsrc ->] | ->]; simpl; split; auto.
Qed.

Lemma let_add_second_oob_behavior_equiv : forall src_result bc_fuel a b,
  Int.min_signed <= a <= Int.max_signed ->
  ~ (Int.min_signed <= b <= Int.max_signed) ->
  (src_result = Term_error constint_malformed_msg \/ src_result = Term_timeout) ->
  behavior_equiv (mk_behavior [] src_result)
...
Proof.
  ...
  - rewrite (rc_step 1 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step 0 [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    destruct Hsrc as [-> | ->]; simpl; split; auto.
  - rewrite (rc_step (S (S fuel)) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s0 s1 [] Hconst_a).
    rewrite (rc_step (S fuel) [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s1 s2 [] Hpush_a).
    rewrite (rc_error fuel [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP] s2 constint_malformed_msg [] Hconst_b).
    destruct Hsrc as [-> | ->]; simpl; split; auto.
Qed.
...
Proof.
  intros x n src_fuel bc_fuel.
  unfold compiler_correct.
  assert (Hcompile :
    compile_program [Decl_expr (Exp_let x (Exp_int n) (Exp_var x))] =
    [CONSTINT n; PUSH; ACC 0; POP 1; STOP]).
  { unfold compile_program; simpl. rewrite String.eqb_refl. reflexivity. }
  rewrite Hcompile.
  ...
  - assert (Hn : Int.min_signed <= n <= Int.max_signed)
    ...
    + change (interpret 0 [Decl_expr (Exp_let x (Exp_int n) (Exp_var x))])
        with (mk_behavior [] Term_timeout).
      apply let_int_var_stop_behavior_equiv; [exact Hn | right; reflexivity].
    ...
    + change (interpret 2 [Decl_expr (Exp_let x (Exp_int n) (Exp_var x))])
      ...
      apply let_int_var_stop_behavior_equiv; [exact Hn | right; reflexivity].
    + replace (S (S (S sf))) with (3 + sf)%nat by lia.
      rewrite (interpret_stable_let_int_var _ _ _ Hn).
      apply let_int_var_stop_behavior_equiv.
      * exact Hn.
      * left. exists (Val_int 0). reflexivity.
  - assert (Hn : ~ (Int.min_signed <= n <= Int.max_signed)).
...
Proof.
  intros x a b src_fuel bc_fuel.
  unfold compiler_correct.
  assert (Hcompile :
    compile_program [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))] =
    [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; STOP]).
  { unfold compile_program; simpl. rewrite String.eqb_refl. reflexivity. }
  rewrite Hcompile.
  destruct (constint_in_range a) eqn:Hrange_a.
  - assert (Ha : Int.min_signed <= a <= Int.max_signed)
    ...
    + assert (Hb : Int.min_signed <= b <= Int.max_signed)
      ...
      * change (interpret 0 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
          with (mk_behavior [] Term_timeout).
        apply let_then_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * change (interpret 1 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
          with (mk_behavior [] Term_timeout).
        apply let_then_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (interpret 2 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
          with (mk_behavior [] Term_timeout)
          by (unfold interpret; simpl; rewrite Hrange_a; reflexivity).
        apply let_then_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (interpret 3 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
          with (mk_behavior [] Term_timeout)
          by (unfold interpret; simpl; rewrite Hrange_a; reflexivity).
        apply let_then_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (S (S (S (S sf)))) with (4 + sf)%nat by lia.
        rewrite (interpret_stable_let_then_add _ _ _ _ Ha Hb).
        apply let_then_add_stop_behavior_equiv.
        -- exact Ha.
        -- exact Hb.
        -- left. exists (Val_int 0). reflexivity.
    + assert (Hb : ~ (Int.min_signed <= b <= Int.max_signed)).
      ...
      * change (interpret 0 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
          with (mk_behavior [] Term_timeout).
        apply let_then_add_second_oob_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * change (interpret 1 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
          with (mk_behavior [] Term_timeout).
        apply let_then_add_second_oob_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (interpret 2 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
      ...
      * replace (interpret 3 [Decl_let x (Exp_int a); Decl_expr (Exp_binop Op_add (Exp_var x) (Exp_int b))])
        ...
        apply let_then_add_second_oob_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (S (S (S (S sf)))) with (4 + sf)%nat by lia.
        rewrite (interpret_stable_let_then_add_b_oob _ _ _ _ Ha Hb).
        apply let_then_add_second_oob_behavior_equiv.
        -- exact Ha.
        -- exact Hb.
        -- left. reflexivity.
  - assert (Ha : ~ (Int.min_signed <= a <= Int.max_signed)).
...
Proof.
  ...
  unfold compiler_correct.
  assert (Hcompile :
    compile_program [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))] =
    [CONSTINT a; PUSH; CONSTINT b; PUSH; ACC 1; ADDINT; POP 1; STOP]).
  { unfold compile_program; simpl. rewrite String.eqb_refl. reflexivity. }
  rewrite Hcompile.
  destruct (constint_in_range a) eqn:Hrange_a.
  - assert (Ha : Int.min_signed <= a <= Int.max_signed)
    ...
    + assert (Hb : Int.min_signed <= b <= Int.max_signed)
      ...
      * change (interpret 0 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
          with (mk_behavior [] Term_timeout).
        apply let_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      ...
      * change (interpret 2 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
          with (mk_behavior [] Term_timeout).
        apply let_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (interpret 3 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
          with (mk_behavior [] Term_timeout)
          by (unfold interpret; simpl; rewrite Hrange_a; reflexivity).
        apply let_add_stop_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (S (S (S (S sf)))) with (4 + sf)%nat by lia.
        rewrite (interpret_stable_let_add _ _ _ _ Ha Hb).
        apply let_add_stop_behavior_equiv.
        -- exact Ha.
        -- exact Hb.
        -- left. exists (Val_int 0). reflexivity.
    + assert (Hb : ~ (Int.min_signed <= b <= Int.max_signed)).
      ...
      * change (interpret 0 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
          with (mk_behavior [] Term_timeout).
        apply let_add_second_oob_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * change (interpret 1 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
          with (mk_behavior [] Term_timeout).
        apply let_add_second_oob_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * change (interpret 2 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
      ...
      * replace (interpret 3 [Decl_expr (Exp_let x (Exp_int a) (Exp_binop Op_add (Exp_var x) (Exp_int b)))])
        ...
        apply let_add_second_oob_behavior_equiv; [exact Ha | exact Hb | right; reflexivity].
      * replace (S (S (S (S sf)))) with (4 + sf)%nat by lia.
        rewrite (interpret_stable_let_add_b_oob _ _ _ _ Ha Hb).
        apply let_add_second_oob_behavior_equiv.
        -- exact Ha.
        -- exact Hb.
        -- left. reflexivity.
  - assert (Ha : ~ (Int.min_signed <= a <= Int.max_signed)).
