 ...
 struct
  ...
  let testbit a n0 =
    (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
      ...
      (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
        (fun _ -> false)
        (fun a0 -> Coq0_Pos.testbit a0 p)
        (fun a0 -> negb (Coq_N.testbit (Coq0_Pos.pred_N a0) p))
        a)
      (fun _ -> false)
      n0
...
let rec of_pos_rec n0 p =
  ...
  if p<=1 then f1 () else if p mod 2 = 0 then f2p (p/2) else f2p1 (p/2))
      ...
      (fun p0 -> lsl0 (of_pos_rec n1 p0) (Uint63.of_int (1)))
...
let of_Z z0 =
  (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
    (fun _ -> (Uint63.of_int (0)))
    (fun p -> of_pos p)
    (fun p -> sub0 (Uint63.of_int (0)) (of_pos p))
    z0
...
let heap_lookup h addr =
  PositiveMap.find (Coq_Pos.of_succ_nat addr) h

(** val heap_alloc : state -> int -> value list -> state * value **)

let heap_alloc s tag fields =
  let addr = s.next_addr in
  let h' = PositiveMap.add (Coq_Pos.of_succ_nat addr) (tag, fields) s.hp in
  ((set (fun s0 -> s0.next_addr) (fun f ->
     let n0 = fun r -> f r.next_addr in
     (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = x.env;
     extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
     x.hp; next_addr = (n0 x) })) (fun _ -> Stdlib.Int.succ addr)
     (set (fun s0 -> s0.hp) (fun f ->
       let h = fun r -> f r.hp in
       (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = x.env;
       extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
       hp = (h x); next_addr = x.next_addr })) (fun _ -> h') s)),
  (Val_ptr addr))

(** val heap_update : heap -> int -> value list -> heap **)

let heap_update h addr fields =
  match PositiveMap.find (Coq_Pos.of_succ_nat addr) h with
  | Some p ->
    let (tag, _) = p in
    PositiveMap.add (Coq_Pos.of_succ_nat addr) (tag, fields) h
  | None -> h
...
| Val_int _ -> None
| Val_block (t0, _) -> Some t0
| Val_ptr addr ->
  (match heap_lookup s.hp addr with
   | Some p -> let (t0, _) = p in Some t0
   | None -> None)
| Val_closure (addr, _) ->
  (match heap_lookup s.hp addr with
   | Some p -> let (t0, _) = p in Some t0
                      ...
                      (match l1 with
                       ...
                       | v1 :: rest ->
                         (match v1 with
                          | Val_int saved_ea ->
                            Step
                              (set (fun s0 -> s0.trap_sp) (fun f ->
                                let n0 = fun r -> f r.trap_sp in
                                (fun x -> { pc = x.pc; accu = x.accu; stack =
                                x.stack; env = x.env; extra_args =
                                x.extra_args; global = x.global; trap_sp =
                                (n0 x); hp = x.hp; next_addr = x.next_addr }))
                                (fun _ -> Z.to_nat prev_tsp)
                                (set (fun s0 -> s0.extra_args) (fun f ->
                                  let n0 = fun r -> f r.extra_args in
                                  (fun x -> { pc = x.pc; accu = x.accu;
                                  stack = x.stack; env = x.env; extra_args =
                                  (n0 x); global = x.global; trap_sp =
                                  x.trap_sp; hp = x.hp; next_addr =
                                  x.next_addr })) (fun _ ->
                                  Z.to_nat saved_ea)
                                  (set (fun s0 -> s0.env) (fun f ->
                                    let v2 = fun r -> f r.env in
                                    (fun x -> { pc = x.pc; accu = x.accu;
                                    stack = x.stack; env = (v2 x);
                                    extra_args = x.extra_args; global =
                                    x.global; trap_sp = x.trap_sp; hp = x.hp;
                                    next_addr = x.next_addr })) (fun _ ->
                                    saved_env)
                                    (set (fun s0 -> s0.stack) (fun f ->
                                      let l2 = fun r -> f r.stack in
                                      (fun x -> { pc = x.pc; accu = x.accu;
                                      stack = (l2 x); env = x.env;
                                      extra_args = x.extra_args; global =
                                      x.global; trap_sp = x.trap_sp; hp =
                                      x.hp; next_addr = x.next_addr }))
                                      (fun _ -> rest)
                                      (set (fun s0 -> s0.accu) (fun f ->
                                        let v2 = fun r -> f r.accu in
                                        (fun x -> { pc = x.pc; accu = 
                                        (v2 x); stack = x.stack; env = x.env;
                                        extra_args = x.extra_args; global =
                                        x.global; trap_sp = x.trap_sp; hp =
                                        x.hp; next_addr = x.next_addr }))
                                        (fun _ -> exn)
                                        (set (fun s0 -> s0.pc) (fun f ->
                                          ...
                                          (fun x -> { pc = (z0 x); accu =
                                          x.accu; stack = x.stack; env =
                                          x.env; extra_args = x.extra_args;
                                          global = x.global; trap_sp =
                                          x.trap_sp; hp = x.hp; next_addr =
                                          x.next_addr })) (fun _ ->
                                          handler_pc) s))))))
                          | _ ->
    ...
    Step
      (set (fun s0 -> s0.stack) (fun f ->
        ...
        (set (fun s0 -> s0.accu) (fun f ->
          ...
          extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
          hp = x.hp; next_addr = x.next_addr })) (fun _ -> val_unit)
          (set (fun s0 -> s0.pc) (fun f ->
            let z0 = fun r -> f r.pc in
            (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env =
            x.env; extra_args = x.extra_args; global = x.global; trap_sp =
...
let handle_PUSH_RETADDR ret_addr pc' s =
  ...
  (set (fun s0 -> s0.stack) (fun f ->
    ...
    (fun x -> { pc = x.pc; accu = x.accu; stack = (l x); env = x.env;
    extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
    x.hp; next_addr = x.next_addr })) (fun _ -> frame)
    (set (fun s0 -> s0.pc) (fun f ->
      let z0 = fun r -> f r.pc in
      (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env = x.env;
      extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
...
let handle_APPLY n0 s =
  match get_code_ptr_s s s.accu with
  | Some target_pc ->
    Step
      (set (fun s0 -> s0.extra_args) (fun f ->
        let n1 = fun r -> f r.extra_args in
        (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = x.env;
        extra_args = (n1 x); global = x.global; trap_sp = x.trap_sp; hp =
        x.hp; next_addr = x.next_addr })) (fun _ ->
        Nat.sub n0 (Stdlib.Int.succ 0))
        (set (fun s0 -> s0.env) (fun f ->
          let v = fun r -> f r.env in
          (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = 
          (v x); extra_args = x.extra_args; global = x.global; trap_sp =
          x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ -> s.accu)
          (set (fun s0 -> s0.pc) (fun f ->
            let z0 = fun r -> f r.pc in
            (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env =
            x.env; extra_args = x.extra_args; global = x.global; trap_sp =
    ...
    (match get_code_ptr_s s s.accu with
     | Some target_pc ->
       ...
       (set (fun s0 -> s0.extra_args) (fun f ->
         let n0 = fun r -> f r.extra_args in
         (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = x.env;
         extra_args = (n0 x); global = x.global; trap_sp = x.trap_sp; hp =
         x.hp; next_addr = x.next_addr })) (fun _ -> 0)
         (set (fun s0 -> s0.env) (fun f ->
           let v = fun r -> f r.env in
           (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env =
           (v x); extra_args = x.extra_args; global = x.global; trap_sp =
       ...
       (match get_code_ptr_s s s.accu with
        | Some target_pc ->
          let new_stack = arg1 :: (arg2 :: ((Val_int
            pc') :: (s.env :: ((Val_int (Z.of_nat s.extra_args)) :: rest))))
          in
          Step
          (set (fun s0 -> s0.extra_args) (fun f ->
            let n0 = fun r -> f r.extra_args in
            (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env =
            x.env; extra_args = (n0 x); global = x.global; trap_sp =
            x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ ->
            Stdlib.Int.succ 0)
            (set (fun s0 -> s0.env) (fun f ->
              let v = fun r -> f r.env in
           ...
           | Some target_pc ->
             ...
             (set (fun s0 -> s0.extra_args) (fun f ->
               ...
               (set (fun s0 -> s0.env) (fun f ->
                 let v = fun r -> f r.env in
                 (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env =
                 (v x); extra_args = x.extra_args; global = x.global;
                 trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                 (fun _ -> s.accu)
                 (set (fun s0 -> s0.stack) (fun f ->
                   let l1 = fun r -> f r.stack in
                   (fun x -> { pc = x.pc; accu = x.accu; stack = (l1 x);
                   env = x.env; extra_args = x.extra_args; global = x.global;
                   trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                   (fun _ -> new_stack)
                   (set (fun s0 -> s0.pc) (fun f ->
...
let handle_APPTERM nargs slotsize s =
...
let handle_APPTERM1 slotsize s =
  match s.stack with
  | [] ->
    Error
...
let handle_APPTERM2 slotsize s =
  match s.stack with
  | [] ->
    Error
        ...
        | Some target_pc ->
          Step
            (set (fun s0 -> s0.extra_args) (fun f ->
              ...
              x.env; extra_args = (n0 x); global = x.global; trap_sp =
              x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ ->
              Nat.add s.extra_args (Stdlib.Int.succ 0))
              (set (fun s0 -> s0.env) (fun f ->
                let v = fun r -> f r.env in
                (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env =
                (v x); extra_args = x.extra_args; global = x.global;
...
let handle_APPTERM3 slotsize s =
  match s.stack with
  | [] ->
    Error
           ...
           | Some target_pc ->
             Step
               (set (fun s0 -> s0.extra_args) (fun f ->
                 ...
                 x.env; extra_args = (n0 x); global = x.global; trap_sp =
                 x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ ->
                 Nat.add s.extra_args (Stdlib.Int.succ (Stdlib.Int.succ 0)))
                 (set (fun s0 -> s0.env) (fun f ->
                   let v = fun r -> f r.env in
                   (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack;
                   env = (v x); extra_args = x.extra_args; global = x.global;
                   trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                   (fun _ -> s.accu)
                   (set (fun s0 -> s0.stack) (fun f ->
                     let l1 = fun r -> f r.stack in
                     ...
                     (set (fun s0 -> s0.pc) (fun f ->
                       ...
                       next_addr = x.next_addr })) (fun _ -> target_pc) s))))
           | None ->
             Error
        ...
        | Some target_pc ->
          Step
            (set (fun s0 -> s0.extra_args) (fun f ->
              ...
              x.env; extra_args = (n0 x); global = x.global; trap_sp =
              x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ ->
              Nat.sub s.extra_args (Stdlib.Int.succ 0))
              (set (fun s0 -> s0.env) (fun f ->
                let v = fun r -> f r.env in
                (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env =
                (v x); extra_args = x.extra_args; global = x.global;
                    ...
                    | Val_int saved_ea ->
                      Step
                        (set (fun s0 -> s0.extra_args) (fun f ->
                          let n0 = fun r -> f r.extra_args in
                          (fun x -> { pc = x.pc; accu = x.accu; stack =
                          x.stack; env = x.env; extra_args = (n0 x); global =
                          x.global; trap_sp = x.trap_sp; hp = x.hp;
                          next_addr = x.next_addr })) (fun _ ->
                          Z.to_nat saved_ea)
                          (set (fun s0 -> s0.env) (fun f ->
                            let v1 = fun r -> f r.env in
                            (fun x -> { pc = x.pc; accu = x.accu; stack =
                            x.stack; env = (v1 x); extra_args = x.extra_args;
                            ...
                            (set (fun s0 -> s0.stack) (fun f ->
                              let l1 = fun r -> f r.stack in
                              (fun x -> { pc = x.pc; accu = x.accu; stack =
                              (l1 x); env = x.env; extra_args = x.extra_args;
                              global = x.global; trap_sp = x.trap_sp; hp =
                              x.hp; next_addr = x.next_addr })) (fun _ ->
                              rest)
                              (set (fun s0 -> s0.pc) (fun f ->
                                let z0 = fun r -> f r.pc in
                                (fun x -> { pc = (z0 x); accu = x.accu;
                                stack = x.stack; env = x.env; extra_args =
                                x.extra_args; global = x.global; trap_sp =
                                x.trap_sp; hp = x.hp; next_addr =
                                x.next_addr })) (fun _ -> ret_pc) s))))
                    | _ ->
                      Error
                        ('R'::('E'::('T'::('U'::('R'::('N'::(':'::(' '::('m'::('a'::('l'::('f'::('o'::('r'::('m'::('e'::('d'::(' '::('r'::('e'::('t'::('u'::('r'::('n'::(' '::('f'::('r'::('a'::('m'::('e'::[])))))))))))))))))))))))))))))))))
           | _ ->
...
let handle_RESTART pc' s =
  let restart_fields = fun all_fields ofs ->
    ...
    in
    let args =
      skipn (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ 0))) fields
    in
                        ...
                        (set (fun s0 -> s0.extra_args) (fun f ->
                          ...
                          (set (fun s0 -> s0.env) (fun f ->
                            ...
                            (set (fun s0 -> s0.stack) (fun f ->
                              ...
                              rest)
                              (set (fun s0 -> s0.accu) (fun f ->
                                let v1 = fun r -> f r.accu in
                                (fun x -> { pc = x.pc; accu = (v1 x); stack =
                                x.stack; env = x.env; extra_args =
                                x.extra_args; global = x.global; trap_sp =
                                x.trap_sp; hp = x.hp; next_addr =
                                x.next_addr })) (fun _ -> closure)
                                (set (fun s0 -> s0.pc) (fun f ->
                                  let z0 = fun r -> f r.pc in
                                  (fun x -> { pc = (z0 x); accu = x.accu;
                                  stack = x.stack; env = x.env; extra_args =
...
let handle_OFFSETCLOSURE ofs pc' s =
  match s.env with
  | Val_block (_, _) ->
    if Z.eqb ofs 0
    then Step
  ...
  | Val_closure (addr, base_ofs) ->
    ...
    (set (fun s0 -> s0.accu) (fun f ->
      ...
      (fun x -> { pc = x.pc; accu = (v x); stack = x.stack; env = x.env;
      extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
      x.hp; next_addr = x.next_addr })) (fun _ -> Val_closure (addr,
      new_ofs))
      (set (fun s0 -> s0.pc) (fun f ->
        let z0 = fun r -> f r.pc in
        (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env = x.env;
...
let handle_PUSHOFFSETCLOSURE ofs pc' s =
  let new_stack = s.accu :: s.stack in
  (match s.env with
   | Val_block (_, _) ->
     if Z.eqb ofs 0
   ...
   | Val_closure (addr, base_ofs) ->
     ...
     (set (fun s0 -> s0.stack) (fun f ->
       ...
       (set (fun s0 -> s0.accu) (fun f ->
         ...
         (fun x -> { pc = x.pc; accu = (v x); stack = x.stack; env = x.env;
         extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
         hp = x.hp; next_addr = x.next_addr })) (fun _ -> Val_closure (addr,
         new_ofs))
         (set (fun s0 -> s0.pc) (fun f ->
           let z0 = fun r -> f r.pc in
           (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env =
...
let handle_GETGLOBALFIELD n0 p pc' s =
  match nth_error s.global n0 with
  | Some glob ->
...
(** val handle_PUSHGETGLOBALFIELD :
    int -> int -> int -> state -> step_result **)

let handle_PUSHGETGLOBALFIELD n0 p pc' s =
  let new_stack = s.accu :: s.stack in
  (match nth_error s.global n0 with
   | Some glob ->
     (match field_or_heap s glob p with
...
let handle_SETGLOBAL n0 pc' s =
  ...
  in
  Step
  (set (fun s0 -> s0.global) (fun f ->
    let l = fun r -> f r.global in
    (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = x.env;
    extra_args = x.extra_args; global = (l x); trap_sp = x.trap_sp; hp =
    x.hp; next_addr = x.next_addr })) (fun _ -> new_global)
    (set (fun s0 -> s0.accu) (fun f ->
      let v = fun r -> f r.accu in
      (fun x -> { pc = x.pc; accu = (v x); stack = x.stack; env = x.env;
      extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
      x.hp; next_addr = x.next_addr })) (fun _ -> val_unit)
      (set (fun s0 -> s0.pc) (fun f ->
        let z0 = fun r -> f r.pc in
        (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env = x.env;
        extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
...
let handle_GETFLOATFIELD n0 pc' s =
  match field_or_heap s s.accu n0 with
  | Some v ->
    Step
      (set (fun s0 -> s0.accu) (fun f ->
          ...
          (match set_nth fields n0 newval with
           | Some new_fields ->
             ...
             (set (fun s0 -> s0.hp) (fun f ->
               ...
               (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env =
               x.env; extra_args = x.extra_args; global = x.global; trap_sp =
               x.trap_sp; hp = (h x); next_addr = x.next_addr })) (fun _ ->
               new_hp)
               (set (fun s0 -> s0.stack) (fun f ->
                 let l = fun r -> f r.stack in
                 (fun x -> { pc = x.pc; accu = x.accu; stack = (l x); env =
                 ...
                 (set (fun s0 -> s0.accu) (fun f ->
                   let v = fun r -> f r.accu in
                   (fun x -> { pc = x.pc; accu = (v x); stack = x.stack;
                   env = x.env; extra_args = x.extra_args; global = x.global;
                   trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                   (fun _ -> val_unit)
                   (set (fun s0 -> s0.pc) (fun f ->
...
let handle_SETFLOATFIELD n0 pc' s =
  match s.stack with
  | [] ->
    Error
        ...
        | Some v0 ->
          Step
            (set (fun s0 -> s0.stack) (fun f ->
              ...
              (set (fun s0 -> s0.accu) (fun f ->
                let v1 = fun r -> f r.accu in
                (fun x -> { pc = x.pc; accu = (v1 x); stack = x.stack; env =
                x.env; extra_args = x.extra_args; global = x.global;
                trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                (fun _ -> v0)
                (set (fun s0 -> s0.pc) (fun f ->
                ...
                (match set_nth fields (Z.to_nat idx) newval with
                 | Some new_fields ->
                   ...
                   (set (fun s0 -> s0.hp) (fun f ->
                     ...
                     (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack;
                     env = x.env; extra_args = x.extra_args; global =
                     x.global; trap_sp = x.trap_sp; hp = (h x); next_addr =
                     x.next_addr })) (fun _ -> new_hp)
                     (set (fun s0 -> s0.stack) (fun f ->
                       let l0 = fun r -> f r.stack in
                       (fun x -> { pc = x.pc; accu = x.accu; stack = 
                       (l0 x); env = x.env; extra_args = x.extra_args;
                       global = x.global; trap_sp = x.trap_sp; hp = x.hp;
                       next_addr = x.next_addr })) (fun _ -> rest)
                       (set (fun s0 -> s0.accu) (fun f ->
                         let v0 = fun r -> f r.accu in
                         (fun x -> { pc = x.pc; accu = (v0 x); stack =
                         x.stack; env = x.env; extra_args = x.extra_args;
                         global = x.global; trap_sp = x.trap_sp; hp = x.hp;
                         next_addr = x.next_addr })) (fun _ -> val_unit)
                         (set (fun s0 -> s0.pc) (fun f ->
                           let z0 = fun r -> f r.pc in
                           (fun x -> { pc = (z0 x); accu = x.accu; stack =
                           x.stack; env = x.env; extra_args = x.extra_args;
       ...
       (match field_or_heap s s.accu (Z.to_nat idx) with
        | Some v0 ->
          (match v0 with
           | Val_int c ->
             Step
               (set (fun s0 -> s0.stack) (fun f ->
                 ...
                 (set (fun s0 -> s0.accu) (fun f ->
                   let v1 = fun r -> f r.accu in
                   (fun x -> { pc = x.pc; accu = (v1 x); stack = x.stack;
                   env = x.env; extra_args = x.extra_args; global = x.global;
                   trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                   (fun _ -> Val_int c)
                   (set (fun s0 -> s0.pc) (fun f ->
           ...
           | _ ->
             Error
               ('G'::('E'::('T'::('S'::('T'::('R'::('I'::('N'::('G'::('C'::('H'::('A'::('R'::(':'::(' '::('i'::('n'::('d'::('e'::('x'::(' '::('o'::('u'::('t'::(' '::('o'::('f'::(' '::('b'::('o'::('u'::('n'::('d'::('s'::(' '::('o'::('r'::(' '::('n'::('o'::('t'::(' '::('a'::(' '::('c'::('h'::('a'::('r'::[])))))))))))))))))))))))))))))))))))))))))))))))))
        | None ->
          Error
            ('G'::('E'::('T'::('S'::('T'::('R'::('I'::('N'::('G'::('C'::('H'::('A'::('R'::(':'::(' '::('i'::('n'::('d'::('e'::('x'::(' '::('o'::('u'::('t'::(' '::('o'::('f'::(' '::('b'::('o'::('u'::('n'::('d'::('s'::(' '::('o'::('r'::(' '::('n'::('o'::('t'::(' '::('a'::(' '::('c'::('h'::('a'::('r'::[])))))))))))))))))))))))))))))))))))))))))))))))))
     | _ ->
                   ...
                   (match set_nth fields (Z.to_nat idx) (Val_int newchar) with
                    | Some new_fields ->
                      ...
                      (set (fun s0 -> s0.hp) (fun f ->
                        ...
                        x.stack; env = x.env; extra_args = x.extra_args;
                        global = x.global; trap_sp = x.trap_sp; hp = 
                        (h x); next_addr = x.next_addr })) (fun _ -> new_hp)
                        (set (fun s0 -> s0.stack) (fun f ->
                          let l0 = fun r -> f r.stack in
                          (fun x -> { pc = x.pc; accu = x.accu; stack =
                          (l0 x); env = x.env; extra_args = x.extra_args;
                          ...
                          (set (fun s0 -> s0.accu) (fun f ->
                            ...
                            (set (fun s0 -> s0.pc) (fun f ->
                              ...
                              x.stack; env = x.env; extra_args =
                              x.extra_args; global = x.global; trap_sp =
                              x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                              (fun _ -> pc') s))))
                    | None ->
                      Error
...
let handle_BRANCH target s =
  Step
    (set (fun s0 -> s0.pc) (fun f ->
      let z0 = fun r -> f r.pc in
      (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env = x.env;
      extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
      x.hp; next_addr = x.next_addr })) (fun _ -> target) s)

(** val handle_BRANCHIF : int -> int -> state -> step_result **)

let handle_BRANCHIF target pc' s =
  match s.accu with
  | Val_int z0 ->
    ((fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
       (fun _ -> Step
       (set (fun s0 -> s0.pc) (fun f ->
         let z1 = fun r -> f r.pc in
         (fun x -> { pc = (z1 x); accu = x.accu; stack = x.stack; env =
         x.env; extra_args = x.extra_args; global = x.global; trap_sp =
         x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ -> pc') s))
       (fun _ -> Step
       (set (fun s0 -> s0.pc) (fun f ->
         ...
         (fun x -> { pc = (z1 x); accu = x.accu; stack = x.stack; env =
         x.env; extra_args = x.extra_args; global = x.global; trap_sp =
         x.trap_sp; hp = x.hp; next_addr = x.next_addr })) (fun _ -> target)
         s))
       (fun _ -> Step
       (set (fun s0 -> s0.pc) (fun f ->
         let z1 = fun r -> f r.pc in
...
let handle_BRANCHIFNOT target pc' s =
  match s.accu with
  | Val_int z0 ->
    ((fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
       (fun _ -> Step
...
(** val handle_SWITCH :
    int -> int -> int list -> int list -> state -> step_result **)

let handle_SWITCH _ _ const_targets block_targets s =
  match s.accu with
  | Val_int n0 ->
  ...
  | Val_block (t0, _) ->
    (match nth_error block_targets t0 with
     | Some target ->
       Step
         (set (fun s0 -> s0.pc) (fun f ->
           let z0 = fun r -> f r.pc in
  ...
  | Val_int z0 ->
    ((fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
       ...
       (set (fun s0 -> s0.accu) (fun f ->
         ...
         (fun x -> { pc = x.pc; accu = (v x); stack = x.stack; env = x.env;
         extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
         hp = x.hp; next_addr = x.next_addr })) (fun _ -> val_true)
         (set (fun s0 -> s0.pc) (fun f ->
           let z1 = fun r -> f r.pc in
           (fun x -> { pc = (z1 x); accu = x.accu; stack = x.stack; env =
           x.env; extra_args = x.extra_args; global = x.global; trap_sp =
       ...
       (set (fun s0 -> s0.accu) (fun f ->
         ...
         (fun x -> { pc = x.pc; accu = (v x); stack = x.stack; env = x.env;
         extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
         hp = x.hp; next_addr = x.next_addr })) (fun _ -> val_false)
         (set (fun s0 -> s0.pc) (fun f ->
           let z1 = fun r -> f r.pc in
           (fun x -> { pc = (z1 x); accu = x.accu; stack = x.stack; env =
           x.env; extra_args = x.extra_args; global = x.global; trap_sp =
...
let handle_PUSHTRAP handler_pc pc' s =
  let prev_tsp = Val_int (Z.of_nat s.trap_sp) in
  let new_stack = (Val_int handler_pc) :: (prev_tsp :: (s.env :: ((Val_int
    (Z.of_nat s.extra_args)) :: s.stack)))
  in
  ...
  (set (fun s0 -> s0.trap_sp) (fun f ->
    let n0 = fun r -> f r.trap_sp in
    (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack; env = x.env;
    extra_args = x.extra_args; global = x.global; trap_sp = (n0 x); hp =
    x.hp; next_addr = x.next_addr })) (fun _ -> new_tsp)
    (set (fun s0 -> s0.stack) (fun f ->
      let l = fun r -> f r.stack in
      (fun x -> { pc = x.pc; accu = x.accu; stack = (l x); env = x.env;
      extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
              ...
              | _ :: rest ->
                Step
                  (set (fun s0 -> s0.trap_sp) (fun f ->
                    ...
                    (fun x -> { pc = x.pc; accu = x.accu; stack = x.stack;
                    env = x.env; extra_args = x.extra_args; global =
                    x.global; trap_sp = (n0 x); hp = x.hp; next_addr =
                    x.next_addr })) (fun _ -> Z.to_nat prev_tsp)
                    (set (fun s0 -> s0.stack) (fun f ->
                      let l2 = fun r -> f r.stack in
                      (fun x -> { pc = x.pc; accu = x.accu; stack = (l2 x);
                      env = x.env; extra_args = x.extra_args; global =
                      x.global; trap_sp = x.trap_sp; hp = x.hp; next_addr =
                      x.next_addr })) (fun _ -> rest)
                      (set (fun s0 -> s0.pc) (fun f ->
                        let z0 = fun r -> f r.pc in
                        (fun x -> { pc = (z0 x); accu = x.accu; stack =
                        x.stack; env = x.env; extra_args = x.extra_args;
...
let handle_C_CALL nargs prim_idx pc' s =
  let args = s.accu :: (firstn (Nat.sub nargs (Stdlib.Int.succ 0)) s.stack) in
...
let handle_PUSHCONSTINT n0 pc' s =
  let new_stack = s.accu :: s.stack in
  Step
  (set (fun s0 -> s0.stack) (fun f ->
    let l = fun r -> f r.stack in
  ...
  | Val_int n0 ->
    Step
      (set (fun s0 -> s0.accu) (fun f ->
        ...
        (fun x -> { pc = x.pc; accu = (v x); stack = x.stack; env = x.env;
        extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
        hp = x.hp; next_addr = x.next_addr })) (fun _ -> Val_int (Z.opp n0))
        (set (fun s0 -> s0.pc) (fun f ->
          let z0 = fun r -> f r.pc in
          (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env =
          x.env; extra_args = x.extra_args; global = x.global; trap_sp =
          ...
          then do_raise div_by_zero_exn s
                 ...
                 (set (fun s0 -> s0.stack) (fun f ->
                   ...
                   (set (fun s0 -> s0.accu) (fun f ->
                     ...
                     env = x.env; extra_args = x.extra_args; global =
                     x.global; trap_sp = x.trap_sp; hp = x.hp; next_addr =
                     x.next_addr })) (fun _ -> Val_int (Z.quot a b))
                     (set (fun s0 -> s0.pc) (fun f ->
                       let z0 = fun r -> f r.pc in
                       (fun x -> { pc = (z0 x); accu = x.accu; stack =
                       x.stack; env = x.env; extra_args = x.extra_args;
          ...
          then do_raise div_by_zero_exn s
                 ...
                 (set (fun s0 -> s0.stack) (fun f ->
                   ...
                   (set (fun s0 -> s0.accu) (fun f ->
                     ...
                     env = x.env; extra_args = x.extra_args; global =
                     x.global; trap_sp = x.trap_sp; hp = x.hp; next_addr =
                     x.next_addr })) (fun _ -> Val_int (Z.rem a b))
                     (set (fun s0 -> s0.pc) (fun f ->
                       let z0 = fun r -> f r.pc in
                       (fun x -> { pc = (z0 x); accu = x.accu; stack =
                       x.stack; env = x.env; extra_args = x.extra_args;
    ...
    Step
      (set (fun s0 -> s0.stack) (fun f ->
        ...
        (set (fun s0 -> s0.accu) (fun f ->
          ...
          if value_phys_eqb s.accu b then val_true else val_false)
          (set (fun s0 -> s0.pc) (fun f ->
            let z0 = fun r -> f r.pc in
            (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env =
            x.env; extra_args = x.extra_args; global = x.global; trap_sp =
    ...
    Step
      (set (fun s0 -> s0.stack) (fun f ->
        ...
        (set (fun s0 -> s0.accu) (fun f ->
          ...
          extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
          hp = x.hp; next_addr = x.next_addr })) (fun _ ->
          if value_phys_eqb s.accu b then val_false else val_true)
          (set (fun s0 -> s0.pc) (fun f ->
            let z0 = fun r -> f r.pc in
            (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env =
            x.env; extra_args = x.extra_args; global = x.global; trap_sp =
...
let handle_ISINT pc' s =
  Step
    (set (fun s0 -> s0.accu) (fun f ->
      ...
      extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp; hp =
      x.hp; next_addr = x.next_addr })) (fun _ ->
      if is_int s.accu then val_true else val_false)
      (set (fun s0 -> s0.pc) (fun f ->
        let z0 = fun r -> f r.pc in
        (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack; env = x.env;
        extra_args = x.extra_args; global = x.global; trap_sp = x.trap_sp;
       ...
       (match l with
        ...
        | tag_val :: rest ->
          ...
          then Step
                 (set (fun s0 -> s0.stack) (fun f ->
                   let l0 = fun r -> f r.stack in
                   (fun x -> { pc = x.pc; accu = x.accu; stack = (l0 x);
                   env = x.env; extra_args = x.extra_args; global = x.global;
                   trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                   (fun _ -> new_stack)
                   (set (fun s0 -> s0.accu) (fun f ->
          ...
          Step
            (set (fun s0 -> s0.stack) (fun f ->
              ...
              (set (fun s0 -> s0.accu) (fun f ->
                ...
                x.env; extra_args = x.extra_args; global = x.global;
                trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                (fun _ -> val_bool (Z.ltb (z_flip_sign a) (z_flip_sign b)))
                (set (fun s0 -> s0.pc) (fun f ->
                  let z0 = fun r -> f r.pc in
                  (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack;
                  env = x.env; extra_args = x.extra_args; global = x.global;
          ...
          Step
            (set (fun s0 -> s0.stack) (fun f ->
              ...
              (set (fun s0 -> s0.accu) (fun f ->
                ...
                x.env; extra_args = x.extra_args; global = x.global;
                trap_sp = x.trap_sp; hp = x.hp; next_addr = x.next_addr }))
                (fun _ -> val_bool (Z.geb (z_flip_sign a) (z_flip_sign b)))
                (set (fun s0 -> s0.pc) (fun f ->
                  let z0 = fun r -> f r.pc in
                  (fun x -> { pc = (z0 x); accu = x.accu; stack = x.stack;
                  env = x.env; extra_args = x.extra_args; global = x.global;
...
let handle_output_char = function
| [] -> Some (Val_int 0)
| _ :: l ->
  (match l with
   | [] -> Some (Val_int 0)
   | v0 :: _ ->
     (match v0 with
      | Val_int c ->
        Some (Val_int
          (print_io
            ((Z.coq_land c ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p)
               ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p)
               ((fun p->1+2*p) 1)))))))) :: [])
            0))
      | _ -> Some (Val_int 0)))
...
let handle_output_bytes = function
| [] -> Some (Val_int 0)
| _ :: l ->
  (match l with
   | [] -> Some (Val_int 0)
                  ...
                  | Val_int len ->
                    let cs =
                      map (fun v ->
                        match v with
                        | Val_int c ->
                          Z.coq_land c ((fun p->1+2*p) ((fun p->1+2*p)
                            ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p)
                            ((fun p->1+2*p) ((fun p->1+2*p) 1)))))))
                        | _ -> 0)
                        (firstn (Z.to_nat len) (skipn (Z.to_nat off) chars))
                    in
  ...
  (match v with
   | Val_int _ ->
     Some (Val_int ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p)
       ...
       ((fun p->1+2*p) 1))))))))))
   | Val_block (t0, _) -> Some (Val_int (Z.of_nat t0))
   | _ -> Some (Val_int 0))
    ...
    | Some filename_codes ->
      ...
      (match D.load_code_section data data_len with
         ...
         (match run fuel code_arr init handler with
          ...
          | Run_error _ ->
            print_io
              ...
              (print_io (((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p)
                1))) :: []) 1)
          | Out_of_fuel _ ->
            print_io
              (str_to_codes
       ...
       | None ->
         print_io
           (str_to_codes
             ('E'::('r'::('r'::('o'::('r'::(':'::(' '::('n'::('o'::(' '::('C'::('O'::('D'::('E'::(' '::('s'::('e'::('c'::('t'::('i'::('o'::('n'::(' '::('f'::('o'::('u'::('n'::('d'::[])))))))))))))))))))))))))))))
           (print_io (((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p) 1))) :: [])
             1))
    | None ->
      print_io
        (str_to_codes
          ('U'::('s'::('a'::('g'::('e'::(':'::(' '::('i'::('n'::('t'::('e'::('r'::('p'::(' '::('<'::('f'::('i'::('l'::('e'::('.'::('b'::('y'::('t'::('e'::('>'::[]))))))))))))))))))))))))))
        (print_io (((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p) 1))) :: []) 1)
 end
...
let z_to_events z0 =
  (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
    ...
    nat_to_events_aux (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
      ...
      (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
      (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
      (Stdlib.Int.succ 0)))))))))))))))))))) (Coq0_Pos.to_nat p) [])
...
let z_to_string z0 =
  (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
    (fun _ -> '0'::[])
    (fun p -> nat_to_string (Coq0_Pos.to_nat p))
    (fun p ->
    append ('('::('-'::[]))
      (append (nat_to_string (Coq0_Pos.to_nat p)) (')'::[])))
    z0
...
| Pat_or (p1, p2) ->
  append ('('::[])
    (append (pp_pattern p1)
      (append (' '::('|'::(' '::[]))) (append (pp_pattern p2) (')'::[]))))
| Pat_record fields ->
  let pp_field = fun f ->
    let (name, p0) = f in
    append name (append (' '::('='::(' '::[]))) (pp_pattern p0))
  in
  append ('{'::(' '::[]))
    (append (intercalate (';'::(' '::[])) (map pp_field fields))
      (' '::('}'::[])))
| Pat_nil -> '['::(']'::[])
| Pat_cons (ph, pt) ->
  append ('('::[])
    (append (pp_pattern ph)
      (append (' '::(':'::(':'::(' '::[]))))
        (append (pp_pattern pt) (')'::[]))))
...
| Exp_field (e0, name) ->
  append ('('::[])
    (append (pp_expr e0) (append ('.'::[]) (append name (')'::[]))))
| Exp_string s -> append ('('::('"'::[])) (append s ('"'::(')'::[])))
| Exp_function cases ->
  let pp_case = fun c ->
    let (p, body) = c in
    append ('|'::(' '::[]))
...
| Td_variant constrs ->
  intercalate (' '::('|'::(' '::[])))
    (map (fun cd ->
      ...
      (match y with
       | Some t0 ->
         append name (append (' '::('o'::('f'::(' '::[])))) (pp_type_expr t0))
       | None -> name))
      constrs)
| Td_alias t0 -> pp_type_expr t0
| Td_record fields ->
  let pp_field = fun f ->
    let (name, t0) = f in
    append name (append (' '::(':'::(' '::[]))) (pp_type_expr t0))
  in
  append ('{'::(' '::[]))
    (append (intercalate (';'::(' '::[])) (map pp_field fields))
      (' '::('}'::[])))
...
| Decl_module (name, decls) ->
  let pp_decl_semi = fun d0 -> append (pp_decl d0) (';'::(';'::[])) in
  append ('m'::('o'::('d'::('u'::('l'::('e'::(' '::[])))))))
    (append name
      (append
        (' '::('='::(' '::('s'::('t'::('r'::('u'::('c'::('t'::(' '::[]))))))))))
        (append (intercalate newline_str (map pp_decl_semi decls))
          (' '::('e'::('n'::('d'::[])))))))
| Decl_open name -> append ('o'::('p'::('e'::('n'::(' '::[]))))) name
| Decl_exception (name, o) ->
  (match o with
   | Some t0 ->
     append
       ('e'::('x'::('c'::('e'::('p'::('t'::('i'::('o'::('n'::(' '::[]))))))))))
...
let rec record_lookup fields f =
  match fields with
  | [] -> None
  | p :: rest ->
    let (name, v) = p in if eqb0 f name then Some v else record_lookup rest f
     ...
     | SVal_record fvs ->
       ...
       | p0 :: rest ->
         ...
         (match record_lookup fvs fname with
          | Some fv ->
            (match match_pattern fp fv with
             | Some e1 ->
               (match match_fields rest with
                | Some e2 -> Some (env_append e1 e2)
                | None -> None)
             | None -> None)
          | None -> None)
...
let apply_builtin b arg out =
  ...
  | Bi_succ ->
    (match arg with
     | SVal_int n0 -> Some ((SVal_int (Z.add n0 1)), out)
     | _ -> None)
  | Bi_pred ->
    (match arg with
     | SVal_int n0 -> Some ((SVal_int (Z.sub n0 1)), out)
     | _ -> None)
  | Bi_max -> None
         ...
         (match s with
          | SVal_record fields ->
            (match record_lookup fields f with
             ...
             | None ->
               Eval_err
                 (('f'::('i'::('e'::('l'::('d'::(' '::('n'::('o'::('t'::(' '::('f'::('o'::('u'::('n'::('d'::[]))))))))))))))),
                 out1))
          | _ ->
            Eval_err
              (('f'::('i'::('e'::('l'::('d'::(' '::('a'::('c'::('c'::('e'::('s'::('s'::(' '::('o'::('n'::(' '::('n'::('o'::('n'::('-'::('r'::('e'::('c'::('o'::('r'::('d'::[])))))))))))))))))))))))))),
              out1))
       | x -> x)
    | Exp_string s -> Eval_ok ((SVal_string s), out)
    | Exp_function cases ->
      Eval_ok ((SVal_closure (('$'::('a'::('r'::('g'::[])))), (Exp_match
...
let rec decl_bound_names = function
| [] -> []
| d :: rest ->
  (match d with
   | Decl_let (x, _) -> x :: (decl_bound_names rest)
   | Decl_letrec (f, _) -> f :: (decl_bound_names rest)
   | _ -> decl_bound_names rest)

(** val add_qualified_bindings :
    ident -> ident list -> env0 -> env0 -> env0 **)

let rec add_qualified_bindings prefix names inner_env env_acc =
  match names with
  | [] -> env_acc
  | x :: rest ->
    let env_acc' =
...
let rec eval_program fuel prog env1 out =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> (env1, (Eval_timeout out)))
    (fun fuel' ->
    match prog with
    | [] -> (env1, (Eval_ok (SVal_unit, out)))
    | d :: rest ->
      (match d with
       | Decl_let (x, e) ->
       ...
       | Decl_module (mod_name, inner_decls) ->
         let (inner_env, other) = eval_program fuel' inner_decls env1 out in
         (match other with
          | Eval_ok (_, out') ->
...
let rec build_field_env fields idx =
  match fields with
  | [] -> []
  | p :: rest ->
    let (name, _) = p in
...
let qualify_name0 prefix name =
  append prefix (append ('.'::[]) name)

(** val decl_bound_names0 : decl list -> ident list **)

let rec decl_bound_names0 = function
| [] -> []
| d :: rest ->
  (match d with
   | Decl_let (x, _) -> x :: (decl_bound_names0 rest)
   | Decl_letrec (f, _) -> f :: (decl_bound_names0 rest)
   | _ -> decl_bound_names0 rest)

(** val prefix_env : ident -> ident list -> int -> comp_env **)

let rec prefix_env prefix names_rev pos =
...
let is_inline_builtin x =
  ...
  then Some ((GETFIELD 0) :: [])
       ...
       else if eqb0 x ('s'::('u'::('c'::('c'::[]))))
            then Some ((OFFSETINT 1) :: [])
            else if eqb0 x
                      ('S'::('t'::('d'::('l'::('i'::('b'::('.'::('I'::('n'::('t'::('.'::('s'::('u'::('c'::('c'::[])))))))))))))))
                 then Some ((OFFSETINT 1) :: [])
                 else if eqb0 x ('p'::('r'::('e'::('d'::[]))))
                      then Some ((OFFSETINT ((~-) 1)) :: [])
                      else if eqb0 x
                                ('S'::('t'::('d'::('l'::('i'::('b'::('.'::('I'::('n'::('t'::('.'::('p'::('r'::('e'::('d'::[])))))))))))))))
                           then Some ((OFFSETINT ((~-) 1)) :: [])
...
| Pat_constr (_, o) -> (match o with
                        | Some p' -> pat_vars p'
                        | None -> [])
| Pat_or (p1, _) -> pat_vars p1
| Pat_cons (ph, pt) -> app (pat_vars ph) (pat_vars pt)
| _ -> []
...
| Exp_if (c, t0, e0) -> app (free_vars c) (app (free_vars t0) (free_vars e0))
...
| Exp_record fields ->
  let rec fv_fields = function
  | [] -> []
  | y :: r -> let (_, e1) = y in app (free_vars e1) (fv_fields r)
  in fv_fields fields
| Exp_field (e1, _) -> free_vars e1
| Exp_function cases ->
  let arg = '$'::('a'::('r'::('g'::[]))) in
  remove_ident arg
    (arg :: (let rec fv_cases = function
             | [] -> []
             | y :: r ->
               let (p, b) = y in
               app (remove_many (pat_vars p) (free_vars b)) (fv_cases r)
             in fv_cases cases))
| Exp_cons (e1, e2) -> app (free_vars e1) (free_vars e2)
| _ -> []
...
let rec compile_expr fuel e ce fe base =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> STOP :: [])
    (fun fuel' ->
    match e with
    ...
    | Exp_binop (op, e1, e2) ->
      ...
      let c1 =
        compile_expr fuel' e1 (shift0 ce (Stdlib.Int.succ 0)) fe
    ...
    | Exp_unop (u, e1) ->
      (match u with
       | Op_neg -> app (compile_expr fuel' e1 ce fe base) (NEGINT :: [])
       | Op_not -> app (compile_expr fuel' e1 ce fe base) (BOOLNOT :: []))
    | Exp_if (cond, then_e, else_e) ->
      let cc = compile_expr fuel' cond ce fe base in
      let cc_len = length cc in
      let ct =
        compile_expr fuel' then_e ce fe
    ...
    | Exp_let (x, e1, e2) ->
      ...
      let c2 =
        compile_expr fuel' e2 ((x, (Loc_stack
          0)) :: (shift0 ce (Stdlib.Int.succ 0))) fe
    ...
    | Exp_letrec (f, e1, e2) ->
      (match e1 with
       | Exp_fun (param, body) ->
         ...
         let body_code =
           compile_expr fuel' body body_env fe (add base (Stdlib.Int.succ 0))
         in
             ...
             | e3 :: l1 ->
               (match l1 with
                | [] ->
                  ...
                  let c2 =
                    compile_expr fuel' e2 (shift0 ce (Stdlib.Int.succ 0)) fe
                      (add (add base c3_len) (Stdlib.Int.succ 0))
                  in
                  let c2_len = length c2 in
                  let c1 =
                    compile_expr fuel' e1
                      (shift0 ce (Stdlib.Int.succ (Stdlib.Int.succ 0))) fe
                      (add
                        ...
                        (match rest with
                         ...
                         | _ :: _ ->
                           ...
                           app ci
                             (app (PUSH :: [])
                               (compile_elems rest
                                 (shift0 ce_acc (Stdlib.Int.succ 0))
                                 (add (add b ci_len) (Stdlib.Int.succ 0))
                                 (Stdlib.Int.succ pushed))))
                    in compile_elems
                  in
    ...
    | Exp_match (scrut, cases) ->
      ...
      in
      let extract_fields =
        let rec extract_fields n0 total scrut_depth =
          (fun fO fS n -> if n=0 then fO () else fS (n-1))
            (fun _ -> [])
            (fun n' ->
            let field_idx = Nat.sub total n0 in
            let rest = extract_fields n' total (Stdlib.Int.succ scrut_depth)
            in
            app ((ACC scrut_depth) :: ((GETFIELD field_idx) :: (PUSH :: [])))
              rest)
            n0
        in extract_fields
      in
      let make_tuple_env =
        let rec make_tuple_env vars idx =
          match vars with
          | [] -> []
          | x :: rest ->
            (x, (Loc_stack
              idx)) :: (make_tuple_env rest (Stdlib.Int.succ idx))
        in make_tuple_env
      in
      let cases_code =
          ...
          | p :: rest ->
            ...
            then (match pat with
                  | Pat_tuple ps ->
                    (match tuple_vars ps with
                     | Some vars ->
                       let nv = length vars in
                       let extr = extract_fields nv nv 0 in
                       let el = length extr in
                       let tup_ce =
                         app (make_tuple_env (rev vars) 0)
                           (shift0 scrut_ce nv)
                       in
                       let bc = compile_expr fuel' body tup_ce fe (add b el)
                       in
                  ...
                  | Pat_cons (ph, pt) ->
                    (match tuple_vars (ph :: (pt :: [])) with
                     | Some vars ->
                       let nv = Stdlib.Int.succ (Stdlib.Int.succ 0) in
                       let extr = extract_fields nv nv 0 in
                       let el = length extr in
                       let cons_ce =
                         app (make_tuple_env (rev vars) 0)
                     ...
                     | Pat_tuple ps ->
                       (match tuple_vars ps with
                        | Some vars ->
                          ...
                          in
                          let bc =
                            compile_expr fuel' body tup_ce fe (add b el)
                          in
                          app extr (app bc ((POP nv) :: []))
                        | None -> compile_expr fuel' body body_ce fe b)
                     | Pat_constr (_, o) ->
                       (match o with
                        | Some cpat ->
                          (match cpat with
                           | Pat_var x ->
                             let extr = (ACC 0) :: ((GETFIELD
                               0) :: (PUSH :: []))
                             in
                             let el = length extr in
                             let constr_ce = (x, (Loc_stack
                               0)) :: (shift0 scrut_ce (Stdlib.Int.succ 0))
                             in
                             let bc =
                               compile_expr fuel' body constr_ce fe (add b el)
                             in
                             app extr
                        ...
                        | Some cpat ->
                          ...
                          (match cpat with
                           | Pat_var x ->
                             ...
                             in
                             let bc =
                               compile_expr fuel' body constr_ce fe
                                 (add bs el)
                             in
                             let bl = length bc in
                             let pop_len = Stdlib.Int.succ 0 in
                             let ns =
                               add (add (add (add bs el) bl) pop_len)
                                 (Stdlib.Int.succ 0)
                             in
                             let rc = compile_cases rest ns in
                             let rl = length rc in
                             ...
                             app test
                               (app ((BRANCHIFNOT (Z.of_nat ns)) :: [])
                                 (app extr
                                   (app bc
                                     (app ((POP (Stdlib.Int.succ 0)) :: [])
                                       (app ((BRANCH (Z.of_nat ep)) :: []) rc)))))
                           | Pat_constr (_, o0) ->
                             (match o0 with
                              | Some inner_pat ->
                                (match inner_pat with
                                 | Pat_var x ->
                                   let extr_outer = (ACC 0) :: ((GETFIELD
                                     0) :: [])
                                   in
                                   let test_inner = PUSH :: ((ACC
                                     0) :: (ISINT :: (BOOLNOT :: [])))
                                   in
                                   let extr_inner = (ACC 0) :: ((GETFIELD
                                     0) :: (PUSH :: []))
                                   in
                                   let setup = app extr_outer test_inner in
                                   let setup_len = length setup in
                                   let bs2 =
                                     add (add bs setup_len) (Stdlib.Int.succ
                                       0)
                                   in
                                   let inner_extr_len = length extr_inner in
                                   let inner_ce = (x, (Loc_stack
                                     0)) :: (shift0 scrut_ce (Stdlib.Int.succ
                                              (Stdlib.Int.succ 0)))
                                   in
                                   let bc =
                                     compile_expr fuel' body inner_ce fe
                                       (add bs2 inner_extr_len)
                                   in
                                   let bl = length bc in
                                   let ns =
                                     add
                                       (add (add (add bs2 inner_extr_len) bl)
                                         (Stdlib.Int.succ 0))
                                       (Stdlib.Int.succ 0)
                                   in
                                   let rc = compile_cases rest ns in
                                     ...
                                     (app ((BRANCHIFNOT (Z.of_nat ns)) :: [])
                                       (app setup
                                         (app ((BRANCHIFNOT
                                           ...
                                           (app extr_inner
                                             (app bc
                                               (app ((POP (Stdlib.Int.succ
                                                 (Stdlib.Int.succ 0))) :: [])
                                                 (app ((BRANCH
                                                   (Z.of_nat ep)) :: []) rc)))))))
                                 | _ ->
                                   let bc =
                                     compile_expr fuel' body body_ce fe bs
                                   in
                                   let bl = length bc in
                                   let ns =
                                     add (add bs bl) (Stdlib.Int.succ 0)
                                   in
                                   let rc = compile_cases rest ns in
                                   let rl = length rc in
                                   let ep = add ns rl in
                                   app test
                                     (app ((BRANCHIFNOT (Z.of_nat ns)) :: [])
                                       (app bc
                                         (app ((BRANCH (Z.of_nat ep)) :: [])
                                           rc))))
                              | None ->
                                let bc = compile_expr fuel' body body_ce fe bs
                                in
                                let bl = length bc in
                                let ns = add (add bs bl) (Stdlib.Int.succ 0)
                                in
                                let rc = compile_cases rest ns in
                                let rl = length rc in
                                let ep = add ns rl in
                     ...
                     | Pat_cons (ph, pt) ->
                       ...
                       (match tuple_vars (ph :: (pt :: [])) with
                        | Some vars ->
                          ...
                          in
                          let bc =
                            compile_expr fuel' body cons_ce fe (add bs el)
                          in
                          let bl = length bc in
                          let pop_len = Stdlib.Int.succ 0 in
                          let ns =
...
let rec compile_decls fuel decls ce fe base =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> STOP :: [])
    (fun fuel' ->
    match decls with
       ...
       | Decl_letrec (f, e) ->
         (match e with
          | Exp_fun (param, body) ->
            ...
            let body_env = make_rec_body_env param f fvs in
            let body_code =
              compile_expr fuel' body body_env fe
                (add base (Stdlib.Int.succ 0))
            in
            let body_instrs =
          ...
          | _ -> compile_decls fuel' ((Decl_let (f, e)) :: rest) ce fe base)
       | Decl_type (_, _, td) ->
         let fe' =
           match td with
           | Td_record fields -> app (build_field_env fields 0) fe
           | _ -> fe
         in
...
let parse_sections0 data data_len =
  let num_sections =
    Z.to_nat
      (read_u32_be data
...
let rec find_section0 secs name =
  match secs with
  | [] -> None
  | s :: rest ->
    if Z.eqb s.sec_name name then Some s else find_section0 rest name
...
let load_code_section0 data data_len =
  let secs = parse_sections0 data data_len in
  (match find_section0 secs cODE_name with
...
let rec read_digits s acc =
  match s with
  | [] -> (acc, s)
  | c::rest ->
    if is_digit c
    then read_digits rest
           (add
             (mul acc (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ 0)))))))))))
             (sub (nat_of_ascii c) (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
               (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
...
let rec read_ident_chars s = match s with
| [] -> ([], s)
| c::rest ->
  if is_ident_char c
  then let (more, remaining) = read_ident_chars rest in ((c::more), remaining)
  else ([], s)
...
| c::rest ->
  if is_ident_start c
  then let (more, remaining) = read_ident_chars rest in
       Some ((c::more), remaining)
  else None
                  ...
                  (match s with
                   ...
                   | c::rest ->
                     ...
                     then (match rest with
                           ...
                           | c'::_ ->
                             if is_ident_char c'
                             then (match parse_ident s with
                                   | Some p ->
                                     let (id, rest') = p in
                                     Some ((Pat_var id), rest')
                                   | None -> None)
                             else Some (Pat_wild, rest))
                     else if is_digit c
                          then (match parse_nat s with
                                | Some p ->
                                  let (n0, rest') = p in
                                  Some ((Pat_int (Z.of_nat n0)), rest')
                                | None -> None)
                          else if is_alpha c
                               then (match parse_ident s with
                                     | Some p ->
                                       ...
                                       if eqb0 id
                                            ('t'::('r'::('u'::('e'::[]))))
                                       then Some ((Pat_bool true), rest')
                                       else if eqb0 id
                                                 ('f'::('a'::('l'::('s'::('e'::[])))))
                                            then Some ((Pat_bool false),
                                                   rest')
                                            else if is_upper c
                                                 then Some ((Pat_constr (id,
                                                        None)), rest')
                                                 else Some ((Pat_var id),
                                                        rest')
                                     | None -> None)
                               else None))))))
    fuel
...
(** val parse_type_expr :
    int -> char list -> (type_expr * char list) option **)

let rec parse_type_expr fuel s =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun fuel' ->
    let parse_paren_type = fun rest1 ->
        ...
        | c::_ ->
          ...
          then (match parse_ident s with
                | Some p ->
                  let (id, rest) = p in
                  if eqb0 id ('i'::('n'::('t'::[])))
                  then Some (Ty_int, rest)
                  else if eqb0 id ('b'::('o'::('o'::('l'::[]))))
                       then Some (Ty_bool, rest)
                       else if eqb0 id ('u'::('n'::('i'::('t'::[]))))
                            then Some (Ty_unit, rest)
...
let rec read_string_contents fuel s acc =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun fuel' ->
    match s with
    ...
    | c::rest ->
      if (=) c '"'
      then Some (acc, rest)
      else read_string_contents fuel' rest (append acc (c::[])))
    fuel

(** val parse_expr : int -> char list -> (expr * char list) option **)

let rec parse_expr fuel s =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun fuel' ->
    match try_neg_int s with
               ...
               (match strip_prefix ('"'::[]) rest1 with
                ...
                | None ->
                  (match strip_prefix
                           ...
                           rest1 with
                   | Some rest2 ->
                     let parse_func_cases =
                       let rec parse_func_cases n0 s0 =
                         (fun fO fS n -> if n=0 then fO () else fS (n-1))
                           (fun _ -> None)
                           (fun n' ->
                                             ...
                                             (match strip_prefix
                                              ...
                                              | Some rest4 ->
                                                ...
                                                (match parse_cases fuel' rest4 with
                                                 | Some p0 ->
                                                   let (cases, rest5) = p0 in
                                                   Some ((Exp_match
                                                   (scrutinee, cases)), rest5)
                                                 | None -> None)
                                              | None -> None)
                                           | None -> None)
                                        | None ->
                     ...
                     then (match parse_nat s with
                          ...
                          then (match parse_ident s with
                                | Some p ->
                                  let (id, rest) = p in
                                  if eqb0 id ('t'::('r'::('u'::('e'::[]))))
                                  then Some ((Exp_bool true), rest)
                                  else if eqb0 id
                                            ('f'::('a'::('l'::('s'::('e'::[])))))
                                       then Some ((Exp_bool false), rest)
                                       else if is_upper c
                                            then Some ((Exp_constr (id,
                                                   None)), rest)
            ...
            (match strip_prefix (' '::('|'::(' '::[]))) rest3 with
             | Some rest4 ->
               (match parse_variant fuel' rest4 with
                | Some p1 ->
                  let (more, rest5) = p1 in
                  Some (((name, (Some t0)) :: more), rest5)
                | None -> None)
         ...
         (match strip_prefix (' '::('|'::(' '::[]))) rest with
          | Some rest2 ->
            (match parse_variant fuel' rest2 with
             | Some p0 ->
               let (more, rest3) = p0 in Some (((name, None) :: more), rest3)
             | None -> None)
...
let rec parse_td_record_fields fuel s =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> None)
    (fun fuel' ->
    match parse_ident s with
...
let parse_type_def fuel s =
  ...
  | None ->
    (match s with
     ...
     | c::_ ->
       if is_upper c
       then (match parse_variant fuel s with
             | Some p ->
               let (constrs, rest) = p in Some ((Td_variant constrs), rest)
             | None -> None)
       else (match parse_type_expr fuel s with
             | Some p -> let (t0, rest) = p in Some ((Td_alias t0), rest)
             | None -> None))
                  ...
                  (match strip_prefix (' '::('='::(' '::[]))) rest'' with
                   | Some rest''' ->
                     (match parse_type_def fuel rest''' with
                      | Some p1 ->
                        let (td, rest4) = p1 in
                        Some ((Decl_type (name, params, td)), rest4)
                      | None -> None)
                   | None -> None)
                | None -> None)
             | None -> None)
               ...
               (match parse_ident rest with
                | Some p ->
                  ...
                  (match strip_prefix
                           ...
                           rest' with
                   | Some rest'' ->
                     let parse_module_decls =
                       let rec parse_module_decls n0 s0 =
                         (fun fO fS n -> if n=0 then fO () else fS (n-1))
                           (fun _ -> None)
                           (fun n' ->
                           ...
                           | None -> None)
                           n0
                       in parse_module_decls
                     in
                     (match parse_module_decls fuel' rest'' with
                      | Some p0 ->
                        let (decls, rest3) = p0 in
                        Some ((Decl_module (name, decls)), rest3)
                      | None -> None)
                   | None -> None)
                | None -> None)
             | None ->
                  ...
                  (match strip_prefix
                        ...
                        (match strip_prefix (' '::('o'::('f'::(' '::[]))))
                         ...
                         | Some rest'' ->
                           (match parse_type_expr fuel rest'' with
                            | Some p0 ->
                              let (t0, rest''') = p0 in
                              Some ((Decl_exception (name, (Some t0))),
                              rest''')
                            | None -> None)
                   ...
                   | None ->
                     (match parse_expr fuel s with
                      | Some p ->
                        let (e, rest) = p in Some ((Decl_expr e), rest)
                      | None -> None)))))))
    fuel
...
let rec parse_program_aux fuel decl_fuel s =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> Some ([], s))
    (fun fuel' ->
    match s with
            ...
            (match strip_prefix newline_str0 rest' with
             | Some rest'' ->
               (match parse_program_aux fuel' decl_fuel rest'' with
                | Some p0 ->
                  let (ds, rest''') = p0 in Some ((d :: ds), rest''')
                | None -> None)
             | None -> Some ((d :: []), rest'))
          | None -> None)
       | None -> None))
...
let lex_parse s =
  let fuel = length0 s in
  (match parse_program_aux fuel fuel s with
   | Some p ->
     let (prog, s0) = p in (match s0 with
                            | [] -> Some prog
                            | _::_ -> None)
   | None -> None)
