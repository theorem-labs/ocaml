...
## Current file layout (post-reorganization)

The generator and its outputs live under `manual/Bytecode/`:

- `manual/Bytecode/generator/` — this plan, `Makefile`, `extract_handlers.sh`,
  and generator-local `gen/` (git-ignored intermediates).
- `manual/Bytecode/Generated/instruct_handlers.v` — `clightgen -normalize`
  output, committed; referenced by `InstructSpec.v`.
- `manual/Bytecode/InstructSpec.v` — `Module Type InstructVerificationSpec`
  (statement-level dependency of every per-handler proof).
- `manual/Bytecode/SatisfiabilityCheck.v` — helper axioms / satisfiability
  of `InstructSpec.v`.
- `automatic/Bytecode/{HandlerLemmas,StepToBigstep}.v` — proof helpers.
- `automatic/Bytecode/InstructVerification/*_correct.v` — 151 per-handler
  proofs.
- `checker/Bytecode/InstructChecker.v` —
  `Module InstructVerification <: InstructVerificationSpec` ascription.
...
Build flow becomes (paths relative to `manual/Bytecode/generator/`):
...
Renames in `manual/Bytecode/Machine.v` and downstream:
...
- `manual/Utils/` — any references.
- `manual/Bytecode/{AST,Machine,Interpret,Encode,InstructSpec,SatisfiabilityCheck}.v`.
- `manual/Bytecode/Generated/instruct_handlers.v` (regenerated by clightgen).
- `automatic/Bytecode/{Decode,HandlerLemmas,StepToBigstep}.v`.
- `automatic/Bytecode/InstructVerification/<OP>_correct.v` — 8 affected handler files.
- `checker/Bytecode/InstructChecker.v`.
- Any references in `checker/common/` (interp_extracted regenerates).
...
All build checks run from `verified-ocaml/` and use the `coq_makefile`
flow (see CLAUDE.md "Verifying Rocq compilation"), not `dune build`.

- After Step 1: `make -C manual/Bytecode/generator Generated/instruct_handlers.v`
  (or whatever target the generator Makefile exposes) must be byte-identical
  to the prior `manual/Bytecode/Generated/instruct_handlers.v` for the 30-odd
...
- After Step 4 (per family): `Print Assumptions correct_OFFSETCLOSURE_ofs.`
  ...
  `checker/Bytecode/InstructChecker.v` must continue to check.
